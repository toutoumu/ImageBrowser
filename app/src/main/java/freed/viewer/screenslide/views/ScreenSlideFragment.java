...
public class ScreenSlideFragment extends Fragment implements ViewPager.OnPageChangeListener, I_OnActivityResultCallback {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        screenslideFragmentBinding =
            DataBindingUtil.inflate(inflater, layout.freedviewer_screenslide_fragment, container, false);
    ...
    private void bind() {
        if (screenslideFragmentBinding != null && screenSlideFragmentModelView != null) {
            ...
            mPagerAdapter = new ScreenSlidePagerAdapter(getChildFragmentManager(),
                                                        screenslideFragmentBinding.pager,
                                                        screenSlideFragmentModelView.getFragmentclickListner());
            ...
            screenSlideFragmentModelView.getFilesHolderModel()
                .addOnPropertyChangedCallback(new Observable.OnPropertyChangedCallback() {
                    @Override
                    public void onPropertyChanged(Observable sender, int propertyId) {
                        if (propertyId == BR.files) {
                            mPagerAdapter.setImageFragmentModels(screenSlideFragmentModelView.getFilesHolderModel()
                                                                     .getImageFragmentModels());
                        }
                    }
                });
            ...
            screenslideFragmentBinding.buttonCloseView.setOnClickListener(v -> {
                if (backClickListner != null) {
                    backClickListner.onButtonClick(screenslideFragmentBinding.pager.getCurrentItem(),
                                                   screenslideFragmentBinding.getRoot());
                ...
                case DialogInterface.BUTTON_POSITIVE:
                    try {
                        screenSlideFragmentModelView.getFilesHolderModel()
                            ...
                            .DeleteFile(screenSlideFragmentModelView.getFolder_to_show());
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                            if (ex instanceof RecoverableSecurityException) {
                                ...
                                try {
                                    startIntentSenderForResult(rex.getUserAction().getActionIntent().getIntentSender(),
                                                               ActivityAbstract.DELETE_REQUEST_CODE,
                                                               null,
                                                               0,
                                                               0,
                                                               0,
    ...
    private View.OnClickListener onplayClick = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            if (screenSlideFragmentModelView.getFolder_to_show() == null) { return; }
            if (!screenSlideFragmentModelView.getFolder_to_show().getName().endsWith(FileEnding.RAW)
                || !screenSlideFragmentModelView.getFolder_to_show().getName().endsWith(FileEnding.BAYER)) {
                Uri uri = null;
                if (screenSlideFragmentModelView.getFolder_to_show() instanceof FileHolder) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                        uri = FileProvider.getUriForFile(getContext(),
                                                         getContext().getPackageName() + ".provider",
                                                         ((FileHolder) screenSlideFragmentModelView.getFolder_to_show()).getFile());
                    } else { uri = Uri.fromFile(((FileHolder) screenSlideFragmentModelView.getFolder_to_show()).getFile()); }
    ...
    private View.OnClickListener onDeleteButtonClick = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.LOLLIPOP
                || Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && !SettingsManager.getInstance()
                .GetWriteExternal()) {
    ...
    public void SetPostition(int position) {
        ...
        if (screenslideFragmentBinding.pager != null) { screenslideFragmentBinding.pager.setCurrentItem(position, false); }
    }
