...
public class DngConvertingFragment extends Fragment {
    ...
    private void setDngProfileToUiItems() {
        ...
        if (filesToConvert != null && filesToConvert.length > 0) {
            ...
            if (dngprofile.toneMapProfile != null) { tmp_name = dngprofile.toneMapProfile.getName(); }
            if (tmp_name == null || TextUtils.isEmpty(tmp_name)) { tmp_name = getString(string.off_); }

            for (int i = 0; i < tonemaps.length; i++)
                if (tonemaps[i].equals(tmp_name)) { toneMapProfile.setSelection(i); }
            ...
            if (dngprofile != null) {
                spinnerMatrixProfile.setOnItemSelectedListener(new OnItemSelectedListener() {
                    ...
                    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                        dngprofile.matrixes =
                            matrixChooserParameter.GetCustomMatrixNotOverWritten(spinnerMatrixProfile.getSelectedItem()
                                                                                     .toString());
                ...
                toneMapProfile.setOnItemSelectedListener(new OnItemSelectedListener() {
                    @Override
                    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                        dngprofile.toneMapProfile = SettingsManager.getInstance()
                            .getToneMapProfiles()
                            .get(toneMapProfile.getSelectedItem().toString());
                    }
    ...
    private void getFilesize(String path) {
        ...
        } else { filesize = new File(path).length(); }
    }
    ...
    private final OnClickListener saveDngProfileClick = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            new XmlParserWriter().saveDngProfiles(SettingsManager.getInstance().getDngProfilesMap(),
                                                  SettingsManager.getInstance().getDeviceString(),
                                                  SettingsManager.getInstance().getAppDataFolder());
                ...
                for (String s : files) {
                    for (BaseHolder b : baseHolders) {
                        if (b.getHolderType() == UriHolder.class) {
                            if (((UriHolder) b).getMediaStoreUri().toString().equals(s)) { convertFiles.add(b); }
    ...
    private Bitmap convertRawToDng(BaseHolder baseHolder) {
        ...
        if (out == null) {
            for (BaseHolder holder : activityInterface.getFileListController().getFiles()) {
                if (holder.getName().equals(file.getName())) { out = holder; }
            }
            if (out == null) { out = activityInterface.getFileListController().getNewImgFileHolder(file); }
        }
        ...
        if ((VERSION.SDK_INT <= VERSION_CODES.LOLLIPOP
            ...
            if (((ActivityInterface) getActivity()).getFileListController().getFreeDcamDocumentFolder() != null
