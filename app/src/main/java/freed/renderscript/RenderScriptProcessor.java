...
public class RenderScriptProcessor implements RenderScriptProcessorInterface, HistogramFeed {
    ...
    public synchronized void Reset(int width, int height, Surface outSurface) {
        ...
        renderScriptManager.SetAllocsTypeBuilder(yuvTypeBuilder,
                                                 rgbTypeBuilder,
                                                 Allocation.USAGE_IO_INPUT | Allocation.USAGE_SCRIPT,
                                                 Allocation.USAGE_IO_OUTPUT | Allocation.USAGE_SCRIPT);

        if (outSurface != null) { renderScriptManager.GetOut().setSurface(outSurface); }
    ...
    private void createHistogramFocusPeakGroup(Builder rgbTypeBuilder) {
        ...
        peakHistoBuilder.addConnection(rgbTypeBuilder.create(),
                                       renderScriptManager.yuvToRgbIntrinsic.getKernelID(),
                                       renderScriptManager.rgb_histogram.getKernelID_processHistogram());
        peakHistoBuilder.addConnection(rgbTypeBuilder.create(),
                                       renderScriptManager.rgb_histogram.getKernelID_processHistogram(),
                                       renderScriptManager.rgb_focuspeak.getFieldID_input());
    ...
    private void createHistogramClippingGroup(Builder rgbTypeBuilder) {
        ...
        peakHistoBuilder.addConnection(rgbTypeBuilder.create(),
                                       renderScriptManager.rgb_histogram.getKernelID_processHistogram(),
                                       renderScriptManager.rgb_clipping.getKernelID_processClipping());
        ...
        clippingHistoGroup.setOutput(renderScriptManager.rgb_clipping.getKernelID_processClipping(),
                                     renderScriptManager.GetOut());
    }
    ...
    private void createClippingFocusPeakGroup(Builder rgbTypeBuilder) {
        ...
        peakHistoBuilder.addConnection(rgbTypeBuilder.create(),
                                       ...
                                       renderScriptManager.rgb_focuspeak.getFieldID_input());
        peakHistoBuilder.addConnection(rgbTypeBuilder.create(),
                                       renderScriptManager.rgb_focuspeak.getKernelID_focuspeak(),
                                       renderScriptManager.rgb_clipping.getKernelID_processClipping());
        ...
        clippingPeakGroup.setOutput(renderScriptManager.rgb_clipping.getKernelID_processClipping(),
                                    renderScriptManager.GetOut());
    }
    ...
    private void createClippingGroup(Builder rgbTypeBuilder) {
        ...
        clipbuilder.addConnection(rgbTypeBuilder.create(),
                                  renderScriptManager.yuvToRgbIntrinsic.getKernelID(),
                                  renderScriptManager.rgb_clipping.getKernelID_processClipping());
        ...
        clippingGroup.setOutput(renderScriptManager.rgb_clipping.getKernelID_processClipping(),
                                renderScriptManager.GetOut());
    }
    ...
    private void createFocusPeakGroup(Builder rgbTypeBuilder) {
        ...
        peakbuilder.addConnection(rgbTypeBuilder.create(),
                                  renderScriptManager.yuvToRgbIntrinsic.getKernelID(),
                                  renderScriptManager.rgb_focuspeak.getFieldID_input());
    ...
    private void createHistogramGroup(Builder rgbTypeBuilder) {
        ...
        histobuilder.addConnection(rgbTypeBuilder.create(),
                                   renderScriptManager.yuvToRgbIntrinsic.getKernelID(),
                                   renderScriptManager.rgb_histogram.getKernelID_processHistogram());
    ...
    private void createAllScriptsGroup(Builder rgbTypeBuilder) {
        ...
        //create the bridge between the kernels
        builder.addConnection(rgbTypeBuilder.create(),
                              renderScriptManager.yuvToRgbIntrinsic.getKernelID(),
                              renderScriptManager.rgb_histogram.getKernelID_processHistogram());
        builder.addConnection(rgbTypeBuilder.create(),
                              renderScriptManager.rgb_histogram.getKernelID_processHistogram(),
        ...
        allScriptGroup.setOutput(renderScriptManager.rgb_clipping.getKernelID_processClipping(),
                                 renderScriptManager.GetOut());
    }
    ...
    class ProcessingTask implements Runnable, OnBufferAvailableListener {
        ...
        public void run() {
            synchronized (workLock) {
                ...
                {
                    peakGroup.execute();
                } else if (processClipping) { clippingGroup.execute(); } else if (processHistogram) // process only histogram
                {
                    if (framescount % HISTOGRAM_UPDATE_RATE == 0) {
                        renderScriptManager.rgb_histogram.invoke_clear();
                        histoGroup.execute();
