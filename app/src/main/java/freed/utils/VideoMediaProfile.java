...
public class VideoMediaProfile {
    ...
    public VideoMediaProfile(CamcorderProfile ex, String ProfileName, VideoMode mode, boolean isAudioActive) {
        ...
        Log.d(TAG,
              "ProfileName:" + ProfileName + "Duration:" + duration + "FileFormat:" + fileFormat + "Quality:" + quality);
        Log.d(TAG,
              "ABR:"
                  + audioBitRate
                  + "AChannels:"
    ...
    public VideoMediaProfile(String t) {
        ...
        if (ar.length == 16) { maxRecordingSize = Long.parseLong(ar[15]); } else { maxRecordingSize = 0; }
    ...
    public VideoMediaProfile(XmlElement xmlElement) {
        ...
        if (xmlElement.findChild("Mode").getValue() == null || xmlElement.findChild("Mode").getValue().isEmpty()) {
            Mode = VideoMode.valueOf("Normal");
        } else {
