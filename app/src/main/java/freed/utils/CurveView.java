...
public class CurveView extends View {
    ...
    private void createControlPoints() {
        ...
        if (drawPointsRects == null) { return; }
        for (int i = 0; i < drawPointsRects.length; i++) {
            if (drawPointsRects[i] != null) {
                if (i == 0) {
                    controlPoints[i].set((drawPointsRects[i + 1].centerX() - drawPointsRects[i].centerX()) / 3,
                                         (drawPointsRects[i + 1].centerY() - drawPointsRects[i].centerY()) / 3);
                } else if (i == drawPointsRects.length - 1) {
                    controlPoints[i].set((drawPointsRects[i].centerX() - drawPointsRects[i - 1].centerX()) / 3,
                                         (drawPointsRects[i].centerY() - drawPointsRects[i - 1].centerY()) / 3);
                } else {
                    controlPoints[i].set((drawPointsRects[i + 1].centerX() - drawPointsRects[i - 1].centerX()) / 3,
                                         (drawPointsRects[i + 1].centerY() - drawPointsRects[i - 1].centerY()) / 3);
                }
            }
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        synchronized (drawLock) {
            ...
            for (int i = 1; i < drawPointsRects.length; i++) {
                if (drawPointsRects[i] != null) {
                    path.cubicTo(
                        drawPointsRects[i - 1].centerX() + controlPoints[i - 1].x,
                        drawPointsRects[i - 1].centerY() + controlPoints[i - 1].y,
                        drawPointsRects[i].centerX() - controlPoints[i].x,
                        drawPointsRects[i].centerY() - controlPoints[i].y,
                        drawPointsRects[i].centerX(),
                        drawPointsRects[i].centerY());
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (drawPointsRects == null) { break; }
                for (int i = 0; i < drawPointsRects.length; i++) {
                    if (drawPointsRects[i] == null) { break; }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (selectedPoint != -1) {
                    ...
                    if (curveChangedListner != null) { curveChangedListner.onCurveChanged(points); }
                    invalidate();
                    return false;
                }
                break;
            case MotionEvent.ACTION_UP:
                if (curveChangedListner != null) {
                    ...
                    } else { curveChangedListner.onClick(points[selectedPoint]); }
                }
                selectedPoint = -1;
                return false;
        }
