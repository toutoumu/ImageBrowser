...
public class StorageFileManager {
    ...
    public String getNewFilePath(boolean externalSd, String fileEnding) {
        ...
        } else { builder.append(fileApiStorageDetector.getInternalSD().getAbsolutePath()); }
        ...
        if (fileEnding.equals(".mp4")) { builder.append(File.separator).append("MOV_"); }
    ...
    private void appendStorageToPath(boolean externalSd, StringBuilder builder) {
        if (externalSd) { builder.append(fileApiStorageDetector.getExternalSD()); } else {
    ...
    public String getNewSessionFolderPath(boolean external) {
        StringBuilder builder = new StringBuilder();
        if (external) { builder.append(fileApiStorageDetector.getExternalSD().getAbsolutePath()); } else {
    ...
    private void readSubFolders(ArrayList<BaseHolder> listToAdd, File folderToRead, boolean external) {
        ...
        if (subfolders != null) {
            for (File f : subfolders) {
                if (!f.isHidden() && f.isDirectory()) { listToAdd.add(new FileHolder(f, external)); }
            }
        }
    }
    ...
    public void readFilesFromFolder(File folder,
                                    List<BaseHolder> list,
                                    FileListController.FormatTypes formatsToShow,
                                    boolean external) {
        ...
        for (File f : folderfiles) {
            if (!f.isHidden()) {
                ...
                )) { list.add(new FileHolder(f, external)); } else if (formatsToShow == FileListController.FormatTypes.dng
                    && f.getName()
                    .toLowerCase()
                    .endsWith(StringUtils.FileEnding.DNG)) { list.add(new FileHolder(f, external)); } else if (formatsToShow
                    == FileListController.FormatTypes.raw && f.getName()
                    .toLowerCase()
                    .endsWith(StringUtils.FileEnding.RAW)) { list.add(new FileHolder(f, external)); } else if (formatsToShow
                    == FileListController.FormatTypes.raw && f.getName()
                    .toLowerCase()
                    .endsWith(StringUtils.FileEnding.BAYER)) { list.add(new FileHolder(f, external)); } else if (
                    formatsToShow == FileListController.FormatTypes.jps
                        && f.getName()
                        .toLowerCase()
                        .endsWith(StringUtils.FileEnding.JPS)) { list.add(new FileHolder(f, external)); } else if (
                    formatsToShow
                        == FileListController.FormatTypes.jpg
                        && f.getName()
                        .toLowerCase()
                        .endsWith(StringUtils.FileEnding.JPG)) { list.add(new FileHolder(f, external)); } else if (
                    formatsToShow
                        == FileListController.FormatTypes.mp4
                        && f.getName()
                        .toLowerCase()
                        .endsWith(StringUtils.FileEnding.MP4)) { list.add(new FileHolder(f, external)); }
            }
        }
        sortFileHolder(list);
    }
