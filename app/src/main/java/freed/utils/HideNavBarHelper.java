...
public class HideNavBarHelper {
    ...
    public void HIDENAVBAR(Window window) {
        if (Build.VERSION.SDK_INT < 16) {
            ...
            decorView.setOnSystemUiVisibilityChangeListener(visibility -> {
                if (visibility > 0) {
                    if (Build.VERSION.SDK_INT >= 16) { window.getDecorView().setSystemUiVisibility(hideflags); }
                }
            });
        }
    }
    ...
    public void showNavbar(Window window) {
        if (Build.VERSION.SDK_INT < 16) {
            ...
            decorView.setOnSystemUiVisibilityChangeListener(visibility -> {
                if (visibility > 0) {
                    if (Build.VERSION.SDK_INT >= 16) { window.getDecorView().setSystemUiVisibility(showflags); }
                }
            });
        }
    }
