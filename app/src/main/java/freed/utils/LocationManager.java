...
public class LocationManager implements LocationListener, LifecycleObserver {
    ...
    public LocationManager(ActivityInterface activityInterface, Lifecycle lifecycle) {
        ...
        locationManager =
            (android.location.LocationManager) FreedApplication.getContext().getSystemService(Context.LOCATION_SERVICE);
    ...
    public void startListing() {
        boolean isON = SettingsManager.getGlobal(SettingKeys.LOCATION_MODE)
            .get()
        ...
        if (isON && permissiongranted) { startLocationListing(); }
    }
    ...
    private void startLocationListing() {
        ...
        if (gps || network) {
            ...
            if (locgps != null) { currentLocation = locgps; } else if (locnet != null) { currentLocation = locnet; }
        } else {
    ...
    public void onLocationChanged(Location location) {
        if (isStarted) { currentLocation = location; } else { currentLocation = null; }
