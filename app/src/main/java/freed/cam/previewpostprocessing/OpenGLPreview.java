...
public class OpenGLPreview implements Preview, TextureView.SurfaceTextureListener {
    ...
    public void setSize(int width, int height) {
        ...
        Log.d(TAG,
              "setSize width :"
                  ...
                  + height
                  + " switch aspectRatio:"
                  + SettingsManager.get(SettingKeys.SWITCH_ASPECT_RATIO).get());
    ...
    public void setHistogram(boolean on) {
        if (on) { histogramController.setFeedToRegister(feed); } else { histogramController.setFeedToRegister(null); }
        histogram = on;
    ...
    public void onSurfaceTextureAvailable(@NonNull SurfaceTexture surface, int width, int height) {
        if (previewEventListner != null) { previewEventListner.onPreviewAvailable(surface, width, height); }
    }

    @Override
    public void onSurfaceTextureSizeChanged(@NonNull SurfaceTexture surface, int width, int height) {
        if (previewEventListner != null) { previewEventListner.onPreviewSizeChanged(surface, width, height); }
    }

    @Override
    public boolean onSurfaceTextureDestroyed(@NonNull SurfaceTexture surface) {
        if (previewEventListner != null) { previewEventListner.onPreviewDestroyed(surface); }
        return false;
    }

    @Override
    public void onSurfaceTextureUpdated(@NonNull SurfaceTexture surface) {
        if (previewEventListner != null) { previewEventListner.onPreviewUpdated(surface); }
    }
