...
public class RenderScriptPreview extends AutoFitTexturviewPreview {
    ...
    public RenderScriptPreview(Context context, HistogramController histogram) {
        ...
        if (RenderScriptManager.isSupported()) { renderScriptManager = new RenderScriptManager(context); }
    ...
    public void setHistogram(boolean on) {
        ishistogram = on;
        if (on) { histogramController.setFeedToRegister(mProcessor); } else { histogramController.setFeedToRegister(null); }
