...
import freed.CustomContainerFragment;
...
public class ActivityFreeDcamMain extends ActivityAbstract
    ...
    public void onFilesChanged() {
        if (uiViewPagerAdapter != null) { uiViewPagerAdapter.updateScreenSlideFile(fileListController.getFiles()); }
    }
    ...
    public void onCameraOpenFinishEvent(CameraStateEvents.CameraOpenFinishEvent cameraOpenFinishEvent) {
        ...
        if (uiViewPagerAdapter == null) { initScreenSlide(); }
        ...
        if (!FileListController.needStorageAccessFrameWork) {
            if (getPermissionManager().isPermissionGranted(PermissionManager.Permissions.SdCard)
                && (fileListController.getFiles() == null || fileListController.getFiles().size() == 0)) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cameraFragmentManager =
            new CameraFragmentManager(getSupportFragmentManager(), id.cameraFragmentHolder, getApplicationContext(), this);
        ...
        bitmapHelper =
            new BitmapHelper(getApplicationContext(), getResources().getDimensionPixelSize(R.dimen.image_thumbnails_size));
    ...
    // FIXME: 2021/3/20 已登录,锁屏的时候进入后台
    @Override
    protected void onStop() {
        super.onStop();
        if (App.isLogin()) {
            Timber.e("ActivityFreeDcamMain进入后台");
            if (CustomContainerFragment.needMoveToBack) {
                // moveTaskToBack(true);
                finish();
            }
        }
    ...
    protected void onResume() {
        ...
        if (!FileListController.needStorageAccessFrameWork) {
            if (getPermissionManager().isPermissionGranted(PermissionManager.Permissions.SdCard_Camera)) {
                if (mSecureCamera != null) { mSecureCamera.onResume(); }
            } else { getPermissionManager().requestPermission(PermissionManager.Permissions.SdCard_Camera); }
        } else {
            if (getPermissionManager().isPermissionGranted(PermissionManager.Permissions.Camera)) {
                if (mSecureCamera != null) { mSecureCamera.onResume(); }
            } else { getPermissionManager().requestPermission(PermissionManager.Permissions.Camera); }
        }
    }
    ...
    public void onResumeTasks() {
        ...
        if (!SettingsManager.getInstance().isInit()) { SettingsManager.getInstance().init(); }

        cameraFragmentManager.onResume();
        if (!SettingsManager.getInstance().appVersionHasChanged() && uiViewPagerAdapter == null) { initScreenSlide(); }

        // FIXME: 2021/3/19 解决旋转不监听问题
    ...
    protected void onPause() {
        ...
        if (mSecureCamera != null) { mSecureCamera.onPause(); }
    }
    ...
    public void onPauseTasks() {
        ...
        if (orientationManager != null) { orientationManager.Stop(); }
        activityIsResumed = false;
    }

    private void initScreenSlide() {
        uiViewPagerAdapter =
            new CameraUiSlidePagerAdapter(getSupportFragmentManager(), onThumbBackClick, screenSlideFragmentModelView);
        if (uiViewPager == null) { uiViewPager = findViewById(id.viewPager_fragmentHolder); }
    ...
    private void unloadCameraFragment() {
        ...
        if (cameraFragmentManager != null) { cameraFragmentManager.unloadCameraFragment(); }
        if (uiViewPagerAdapter != null) { uiViewPager.post(() -> {uiViewPagerAdapter.setCameraFragment(null);}); }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (activityIsResumed && (cameraFragmentManager.getCameraFragment() != null
            && cameraFragmentManager.getCameraFragment().getCamera() != null
            && cameraFragmentManager.getCameraFragment().getCamera().getParameterHandler() != null)) {
            Log.d(TAG, "KeyCode Pressed:" + keyCode);
            int appSettingsKeyShutter = 0;

            try {
                String es = cameraFragmentManager.getCameraFragment()
                    .getCamera()
                    .getParameterHandler()
                    .get(SettingKeys.EXTERNAL_SHUTTER)
                    .GetStringValue();
                if (es == null) { super.onKeyDown(keyCode, event); }
                if (es.equals("Vol+")) { appSettingsKeyShutter = KeyEvent.KEYCODE_VOLUME_UP; } else if (es.equals("Vol-")) {
                    appSettingsKeyShutter = KeyEvent.KEYCODE_VOLUME_DOWN;
                } else if (es.equals(
                    "Hook")
                    || es.isEmpty()) { appSettingsKeyShutter = KeyEvent.KEYCODE_HEADSETHOOK; }
            } catch (NullPointerException ex) {
                Log.WriteEx(ex);
            }

            if ((
                // FIXME: 2021/3/19 去掉这个判断, 解决自动拍摄问题
                ...
                && (cameraFragmentManager.getCameraFragment() != null
                && cameraFragmentManager.getCameraFragment().getCamera() != null)) {
    ...
    private final ScreenSlideFragment.ButtonClick onThumbBackClick = new ScreenSlideFragment.ButtonClick() {
        ...
        public void onButtonClick(int position, View view) {
            //show cameraui
            if (uiViewPager != null) { uiViewPager.setCurrentItem(1); }
        }
    };

    @Override
    public void SetNightOverlay() {
        if (nightoverlay == null) { nightoverlay = findViewById(id.nightoverlay); }
        Log.d(TAG, "NightOverlay:" + SettingsManager.getGlobal(SettingKeys.NightOverlay).get());
        if (SettingsManager.getGlobal(SettingKeys.NightOverlay).get()) { nightoverlay.setVisibility(View.VISIBLE); } else {
