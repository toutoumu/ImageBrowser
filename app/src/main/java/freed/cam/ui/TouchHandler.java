...
public class TouchHandler {
    ...
    private boolean detectSwipeDirection() {
        ...
        if (swipeDetected || newActionBlocked) { return false; }
        ...
        if (x >= distance || y >= distance) {
            ...
            if (x >= y) {
                if (currentX > startX) { doLeftToRightSwipe(); } else { doRightToLeftSwipe(); }
            } else {
                if (currentY > startY) { doTopToBottomSwipe(); } else { doBottomToTopSwipe(); }
            }
            newActionBlocked = true;
            handler.postDelayed(resetActionBlock, blockTime);
            return false;
