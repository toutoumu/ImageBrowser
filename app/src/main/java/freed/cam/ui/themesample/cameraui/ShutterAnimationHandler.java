...
public class ShutterAnimationHandler extends Handler {
    ...
    private void run() {
        ...
        while (shutteractive) {
            ...
            if (drawTimer) { shutteropentime = getTimeGoneString(startime); } else { shutteropentime = ""; }
    ...
    private void draw() {
        ...
        switch (currentShow) {
            case video_recording_stop:
                ...
                if (recordingRadiusRectangle < 0 || currentframe == MAXFRAMES) { recordingRadiusRectangle = 0; }
                drawRecordingImage = true;
                break;
            case video_recording_start:
                shutter_open_radius = 0;
                recordingRadiusCircle -= RECORDING_OPEN_STEP;
                if (recordingRadiusCircle < 0 || currentframe == MAXFRAMES) { recordingRadiusCircle = 0; }
            ...
            case image_capture_stop:
                ...
                if (shutter_open_radius < 0 || currentframe == MAXFRAMES) { shutter_open_radius = 0; }
                break;
            ...
            case continouse_capture_work_start:
                drawRecordingImage = true;
                shutter_open_radius += SHUTTER_OPEN_STEP;
                if (shutter_open_radius > MAX_SHUTTER_OPEN) { shutter_open_radius = MAX_SHUTTER_OPEN; }
