...
public class ManualButton extends LinearLayout {
    ...
    public void onViewStateChanged(ValueChangedEvent<AbstractParameter.ViewState> viewStateValueChangedEvent) {
        if (viewStateValueChangedEvent.type != AbstractParameter.ViewState.class) { return; }
    ...
    private void applyViewState(AbstractParameter.ViewState state) {
        ...
        switch (state) {
            case Enabled:
                ...
                if (imageView != null) { imageView.setColorFilter(Color.TRANSPARENT, PorterDuff.Mode.SRC_ATOP); }
                break;
            case Disabled:
                ManualButton.this.setVisibility(VISIBLE);
                ManualButton.this.setEnabled(false);
                if (imageView != null) { imageView.setColorFilter(Color.GRAY, PorterDuff.Mode.SRC_ATOP); }
                break;
            case Visible:
    ...
    public void onIntValueChanged(ValueChangedEvent<Integer> current) {
        if (current.type != Integer.class) { return; }
        if (current.key == parameter.getKey()) {
            ...
            if (txt != null && !TextUtils.isEmpty(txt) && !txt.equals("null")) { valueTextView.setText(txt); } else {
    ...
    public void onValuesChanged(ValueChangedEvent<String[]> values) {
        if (values.type != String[].class) { return; }
    ...
    public void onStringValueChanged(ValueChangedEvent<String> value) {
        if (value.type != String.class) { return; }
        if (value.key == parameter.getKey()) { valueTextView.setText(value.newValue); }
    }
    ...
    public void SetManualParameter(ParameterInterface parameter) {
        ...
        if (parameter != null) {
            ...
            if (valueTextView != null) {
                if (txt != null && !TextUtils.isEmpty(txt)) { valueTextView.setText(txt); } else {
    ...
    private String getStringValue(int pos) {
        if (parameterValues != null && parameterValues.length > 0) {
            if (pos >= parameterValues.length) { return parameterValues[parameterValues.length - 1]; } else if (pos < 0) {
                return parameterValues[0];
            } else { return parameterValues[pos]; }
        }
    ...
    public String[] getStringValues() {
        if (parameterValues == null || parameterValues.length == 0) { createStringParametersStrings(parameter); }
