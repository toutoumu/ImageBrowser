...
public class FocusImageHandler extends AbstractFocusImageHandler {
    ...
    public void FocusFinished(final boolean success) {
        // FIXME: 2018/3/23 添加点击拍摄功能
        if (App.isLogin() && success && !SettingsManager.getGlobal(SettingKeys.TouchToCapture).get()) {
            // 添加对焦后拍摄功能,只有自动对焦和微距才需要在这里调用,其他对焦方式在OnClick方法里面调用
            String focusMode = wrapper.getParameterHandler().get(SettingKeys.FocusMode).GetStringValue();
            if (focusMode.equalsIgnoreCase("auto") || focusMode.equalsIgnoreCase("macro")) {
                wrapper.getModuleHandler().getCurrentModule().DoWork();
            }
        }
                ...
                focusImageView.post(() -> {
                    ...
                    Log.d(TAG,
                          "Focus success:"
                              + success
                              + " TouchtoCapture:"
                              + SettingsManager.getGlobal(SettingKeys.TouchToCapture).get());
                    if (success && SettingsManager.getGlobal(SettingKeys.TouchToCapture).get() && !wrapper.getModuleHandler()
                        .getCurrentModule()
                        .ModuleName()
                        .equals(FreedApplication.getStringFromRessources(R.string.module_video))) {
    ...
    public void FocusLocked(final boolean locked) {
        cancelFocus.post(() -> {
            if (locked) { cancelFocus.setVisibility(View.VISIBLE); } else { cancelFocus.setVisibility(View.GONE); }
        });
    }
    ...
    public void TouchToFocusSupported(boolean isSupported) {
        ...
        if (!isSupported) { focusImageView.setVisibility(View.GONE); }
    }
    ...
    public void AEMeteringSupported(final boolean isSupported) {
        ...
        meteringArea.post(() -> {
            if (isSupported) { meteringArea.setVisibility(View.VISIBLE); } else { meteringArea.setVisibility(View.GONE); }
        });
    }
    ...
    private final I_TouchListnerEvent meteringTouch = new I_TouchListnerEvent() {
        ...
        public void onAreaCHanged(int x, int y, int previewWidth, int previewHeight) {
            if (wrapper != null) { wrapper.getFocusHandler().SetMeteringAreas(x, y, previewWidth, previewHeight); }
        }
        ...
        public void IsMoving(boolean moving) {
            ...
            if (moving
                && expolock != null
                && expolock.getViewState() == AbstractParameter.ViewState.Visible
                && expolock.GetStringValue().equals("true")) {
    ...
    public void OnClick(int x, int y) {
        // FIXME: 2018/3/23 固定对焦和白平衡位置
        if (App.isLogin()) {
            if (wrapper == null || wrapper.getFocusHandler() == null) return;
            // API1 如果相机还未准备好
            if (wrapper instanceof Camera1Fragment/* && !wrapper.getCameraHolder() .IsRdy()*/) {
                return;
            }
            // 如果正在拍摄那么不允许点击
            if (wrapper.getModuleHandler().getCurrentModule().IsWorking()) {
                return;
            }
            // 如果对焦模式不是微距或自动那么直接调用拍摄 infinity continuous-picture
            // 微距(auto)和自动(auto)是在对焦回调里面调用拍照 FocusFinished
            String focusMode = wrapper.getParameterHandler().get(SettingKeys.FocusMode).GetStringValue().toLowerCase();
            if (!focusMode.equalsIgnoreCase("auto") && !focusMode.equalsIgnoreCase("macro")) {
                wrapper.getModuleHandler().getCurrentModule().DoWork();
                return;
            }
            // 手机横放左上角为原点 x轴向下,y轴向右
            if (wrapper.getActivityInterface().getOrientation() == 270) {
                //设置对焦区域
                wrapper.getFocusHandler().StartTouchToFocus(540, 540, 1920, 1080);
            } else {
                //设置对焦区域
                wrapper.getFocusHandler().StartTouchToFocus(1390, 540, 1920, 1080);
            }
            return;
        }
        ...
        if (wrapper.getFocusHandler() != null) { wrapper.getFocusHandler().StartTouchToFocus(x, y, disWidth, disHeight); }
    }
    ...
    private Rect centerImageView(ImageView imageview) {
        ...
        return new Rect((int) imageview.getX() - recthalf,
                        (int) imageview.getX() + recthalf,
                        (int) imageview.getY() - recthalf,
                        (int) imageview.getY() + recthalf);
    }
}
