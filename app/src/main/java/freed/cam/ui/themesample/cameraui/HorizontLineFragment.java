...
public class HorizontLineFragment extends AbstractFragment implements ParameterEvents {
    ...
    public void onStringValueChanged(String value) {
        // FIXME: 2018/3/23 解决参数不能持久化问题
        SettingsManager.getGlobal(SettingKeys.HorizontLvl).set(value);
        if (SettingsManager.getGlobal(SettingKeys.HorizontLvl).get() != null
            && SettingsManager.getGlobal(SettingKeys.HorizontLvl)
            .get()
            .equals(FreedApplication.getStringFromRessources(string.on))) {
            startSensorListing();
    ...
    private void startSensorListing() {
        if (SettingsManager.getGlobal(SettingKeys.HorizontLvl)
            .get()
            .equals(FreedApplication.getStringFromRessources(string.on))) {
    ...
    private void stopSensorListing() {
        if (sensorManager != null) { sensorManager.unregisterListener(msl); }
    }
    ...
    public void onResume() {
        ...
        try {
            if (SettingsManager.getGlobal(SettingKeys.HorizontLvl).get() != null
                && SettingsManager.getGlobal(SettingKeys.HorizontLvl)
                .get()
                .equals(FreedApplication.getStringFromRessources(string.off))
                || TextUtils.isEmpty(SettingsManager.getGlobal(SettingKeys.HorizontLvl).get())) {
                view.setVisibility(View.GONE);
            } else { startSensorListing(); }
    ...
    private void updateUi(final float pitch, final float roll, final float yaw) {
        handler.post(() -> {
            ...
            if (pitchdegree > -89) {
                if (upImage.getVisibility() != View.VISIBLE) { upImage.setVisibility(View.VISIBLE); }
                downImage.setVisibility(View.GONE);
            } else if (pitchdegree < -91) {
                upImage.setVisibility(View.GONE);
                if (downImage.getVisibility() != View.VISIBLE) { downImage.setVisibility(View.VISIBLE); }
