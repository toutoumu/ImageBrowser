...
public class RotatingSeekbar extends View {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case ONPROGRESSCHANGED:
                    if (mListener != null) { mListener.onProgressChanged(null, msg.arg1, true); }
                    break;
                case ONHANDELAUTOSCROLL:
                    ...
                    if (positivepos <= realMax && positivepos >= realMin) {
                        ...
                        if (rerun) { handleAutoScroll(); }
                    } else {
                        autoscroll = false;
                        distanceInPixelFromLastSwipe = 0;
                        if (positivepos > realMax) { setProgress(Values.length - 1, true); } else if (positivepos
                            < realMin) { setProgress(0, true); } else {
                            checkifCurrentValueHasChanged();
                            if (currentValue > Values.length - 1) { currentValue = Values.length - 1; }
    ...
    private String[] Values =
        "Auto,1/100000,1/6000,1/4000,1/2000,1/1000,1/500,1/250,1/125,1/60,1/30,1/15,1/8,1/4,1/2,2,4,8,15,30,60,180".split(",");
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        viewHeight = h;
        topToHalfGradient =
            new LinearGradient(0, 0, 0, viewHeight / 2, Color.TRANSPARENT, Color.BLACK, Shader.TileMode.MIRROR);
        halfToBottmGradient =
            new LinearGradient(0, viewHeight / 2, 0, viewHeight, Color.BLACK, Color.TRANSPARENT, Shader.TileMode.MIRROR);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < Values.length; i++) {
            ...
            if (dif <= VISIBLE_ITEMS_INVIEW / 2) {
                ...
                canvas.drawLine(viewWidth - convertDpiToPixel(20),
                                xpos - textsize / 2,
                                viewWidth - 20,
                                xpos - textsize / 2,
                                paint);
                if (null != val) { canvas.drawText(val, viewWidth / 2 + convertDpiToPixel(10), xpos, paint); }
            }
        }
        paint.setAlpha(255);
        paint.setStrokeWidth(10);
        canvas.drawLine(viewWidth - convertDpiToPixel(10),
                        viewHeight / 2 + itemHeight / 2,
                        viewWidth,
                        viewHeight / 2 + itemHeight / 2,
                        paint);
    }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (sliderMoving) {
                    ...
                    throwevent = false;
                    if (distanceInPixelFromLastSwipe > 0 && distanceInPixelFromLastSwipe > 10
                        || distanceInPixelFromLastSwipe < 0 && distanceInPixelFromLastSwipe < -10) {
                        autoscroll = true;
                        handleAutoScroll();
                    } else {
                        setProgress(currentValue, true);
    ...
    private void checkifCurrentValueHasChanged() {
        ...
        if (item != currentValue) {
            ...
            currentValue = item;
            if (currentValue >= Values.length) { currentValue = Values.length - 1; }
            if (currentValue < 0) { currentValue = 0; }
    ...
    public void setProgress(int progress, boolean throwevent) {
        ...
        handler.setINVALIDATE();
        if (throwevent) { handler.setONPROGRESSCHANGED(currentValue); }
    }
