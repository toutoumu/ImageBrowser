...
public class SettingsChildMenuTimeLapseFrames extends LinearLayout {
    ...
    private void init(Context context) {
        ...
        minus.setOnClickListener(v -> {
            ...
            if (current - bigmover >= 1) { current -= bigmover; } else if (current - mover > min) { current -= mover; }
            setCurrent(current);
        });
        plus.setOnClickListener(v -> {
            if (current + mover > 1 && current + bigmover <= max) { current += bigmover; } else if (current + mover <= 1) {
                current += mover;
            }
            setCurrent(current);
        });
        ...
        if (fps == null || TextUtils.isEmpty(fps) || fps.equals("null")) { fps = "30"; }

        Log.d(TAG, "set to " + fps);
        current = Float.parseFloat(fps);
        if (current >= 1) { editText.setText(current + " fps"); } else { editText.setText(current * 60 + " fpm"); }
    }
