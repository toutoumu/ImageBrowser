 ...
 */
public class CameraUiFragment extends AbstractFragment
    implements SettingsChildAbstract.SettingsChildClick, SettingsChildAbstract.CloseChildClick, I_swipe, OnClickListener {
    ...
    public void setCameraToUi(CameraWrapperInterface wrapper) {
        ...
        if (cameraUiWrapper == null) {
            if (focusImageHandler != null) {
                ...
                if (isAdded()) { hide_ManualSettings(); }
            }
        } else {
            if (App.isLogin() && customContainerFragment != null) {
                customContainerFragment.setWrap(wrapper);
            }
            ...
            if (cameraUiWrapper.getParameterHandler().get(SettingKeys.PictureFormat) != null) {
                setUiItem(left_ui_items_holder,
                          parameterHandler.get(SettingKeys.PictureFormat),
                          R.drawable.quck_set_format2);
            }
            ...
            if (isAdded()) {
                ...
                if (manualModesFragment != null) { manualModesFragment.setCameraToUi(cameraUiWrapper); }
                ...
                if (SettingsManager.getGlobal(SettingKeys.SHOWMANUALSETTINGS).get()) { showManualSettings(); }
                //remove the values fragment from ui when a new api gets loaded and it was open.
                if (horizontalValuesFragment != null && horizontalValuesFragment.isAdded()) { removeHorizontalFragment(); }
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        if ((getActivity() != null) && (((ActivityFreeDcamMain) getActivity()).getUserMessageHandler() != null)) {
            ((ActivityFreeDcamMain) getActivity()).getUserMessageHandler()
                .setMessageTextView(view.findViewById(id.textView_usermessage), view.findViewById(id.userMessageHolder));
        }
     ...
     * 自定义覆盖在相机上面的黑色遮罩
     *
     * showHelp 当前是否显示了帮助界面
     */
    private void custom_container() {
        boolean showHelp = SettingsManager.getInstance().getShowHelpOverlay();
        if (!showHelp && App.isLogin()) {//如果已经登录且没有显示帮助界面
            FragmentTransaction transaction = getChildFragmentManager().beginTransaction();
            transaction.setCustomAnimations(anim.empty, anim.empty);
            customContainerFragment.setWrap(cameraUiWrapper);
            transaction.replace(id.helpfragment_container, customContainerFragment);
            transaction.addToBackStack(null);
                            ...
                            versionView.addView(new VersionView(getContext(), new VersionView.ButtonEvents() {
                                ...
                                public void onDownloadClick() {
                                    startActivity(Intent.createChooser(new Intent(Intent.ACTION_VIEW,
                                                                                  Uri.parse(
                                                                                      "https://github.com/KillerInk/FreeDcam/releases/latest")),
                                                                       "Choose browser"));
                                ...
                                public void onCloseClick() {
                                    try {
                                        if (versionView != null) { versionView.removeAllViews(); }
    ...
    public void onSettingsChildClick(UiSettingsChild item, boolean fromLeftFragment) {
        ...
        if (horizontalValuesFragment != null) { horizontalValuesFragment.Clear(); }
        ...
        if (fromLeftFragment) { params.addRule(RelativeLayout.ALIGN_PARENT_LEFT); } else {
        ...
        if (tmo != null && tmo.length > 0) { horizontalValuesFragment.SetStringValues(tmo, this); }
    ...
    private void removeHorizontalFragment() {
        getChildFragmentManager().beginTransaction()
            .remove(horizontalValuesFragment)
            .setCustomAnimations(0, anim.right_to_left_exit)
            .commit();
    }
    ...
    public void onClick(int x, int y) {
        if (focusImageHandler != null) { focusImageHandler.OnClick(x, y); }
    }

    @Override
    public void onMotionEvent(MotionEvent event) {
        if (focusImageHandler != null) { focusImageHandler.onTouchEvent(event); }
    }
