...
public abstract class AbstractInfoOverlayHandler {
    ...
    private void startLooperThread() {
        if (started) { handler.postDelayed(runner, 1000); }
    }
    ...
    private void getFormat() {
        if (cameraUiWrapper.getModuleHandler()
            ...
            if (videoprofile != null) { size = videoprofile.GetStringValue(); } else { size = ""; }
        } else {
            ParameterInterface pictureFormat = cameraUiWrapper.getParameterHandler().get(SettingKeys.PictureFormat);
            if (pictureFormat != null) { format = pictureFormat.GetStringValue(); } else { format = ""; }

            ParameterInterface pictureSize = cameraUiWrapper.getParameterHandler().get(SettingKeys.PictureSize);
            if (pictureSize != null) { size = pictureSize.GetStringValue(); } else { size = ""; }
        }
    }
    ...
    private void getStorageSpace() {
        try {
            ...
            } else { storageSpace = readableFileSize(SDspace()); }
        } catch (Exception ex) {
            storageSpace = "";
        }
    }
    ...
    private double Calc() {
        ...
        if (SettingsManager.get(SettingKeys.PictureFormat)
            .get()
            .contains(FreedApplication.getStringFromRessources(R.string.bayer_))) {
            if (Build.MANUFACTURER.contains("HTC")) {
                return Integer.parseInt(res[0]) * 2 * Integer.parseInt(res[1]) * 16 / 8;
            } else { return Integer.parseInt(res[0]) * Integer.parseInt(res[1]) * 10 / 8; }
        } else { return Integer.parseInt(res[0]) * Integer.parseInt(res[1]) * 8 / 8; }
    }
    ...
    private long SDspace() {
        ...
        if (!SettingsManager.getInstance().GetWriteExternal()) {
            ...
            if (VERSION.SDK_INT > 17) { bytesAvailable = stat.getFreeBytes(); } else {
