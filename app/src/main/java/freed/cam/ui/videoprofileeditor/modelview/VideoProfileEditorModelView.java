...
public class VideoProfileEditorModelView extends ViewModel {
    ...
    private void updateModels() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            } else { encoderModel.setTxt(currentProfile.get().encoderName); }
            ...
            if (currentProfile.get().level == -1 && currentProfile.get().profile == -1) {
                ...
                String t = null;
                t = MediaCodecInfoParser.getProfileString(translateVideoCodecString(), currentProfile.get().profile)
                    + " "
                    + MediaCodecInfoParser.getLevelString(translateVideoCodecString(), currentProfile.get().level);
    ...
    private String translateVideoCodecString() {
        if (videoCodecModel.getTxt().equals(VideoCodecs.HEVC.name())) { return MediaCodecInfoParser.hevc; }
        if (videoCodecModel.getTxt().equals(VideoCodecs.H264.name())) { return MediaCodecInfoParser.avc; }
        return "";
    }
