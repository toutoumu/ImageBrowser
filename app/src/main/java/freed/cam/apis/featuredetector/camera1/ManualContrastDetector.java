...
public class ManualContrastDetector extends BaseParameter1Detector {
    ...
    private void detectManualContrast(Camera.Parameters parameters) {
        if (SettingsManager.getInstance().getFrameWork() == Frameworks.MTK) {
            if (parameters.get(camstring(R.string.contrast)) != null
                && parameters.get(camstring(R.string.contrast_values)) != null) {
                SettingsManager.get(SettingKeys.M_Contrast)
                    .setValues(parameters.get(camstring(R.string.contrast_values)).split(","));
            ...
            try {
                ...
                if (parameters.get(camstring(R.string.contrast_max)) != null) {
                    ...
                    SettingsManager.get(SettingKeys.M_Contrast)
                        .setCamera1ParameterKEY("contrast"); // constrast is not a typo. on huawei side it is
                    SettingsManager.get(SettingKeys.M_Contrast).setIsSupported(true);
