...
public class Camera1FeatureDetectorTask extends AbstractFeatureDetectorTask {
    ...
    public List<String> findCameraIDs() {
        ...
        for (int i = 0; i < 200; i++) {
            ...
            try {
                ...
                if (parameters != null) { cam_ids.add(String.valueOf(i)); }
    ...
    public static String[] createIsoValues(int miniso, int maxiso, int step, boolean xiaomi) {
        ...
        for (int i = miniso; i <= maxiso; i += step) {
            if (xiaomi) { s.add("ISO" + i); } else { s.add(i + ""); }
        }
    ...
    private void detectFrontCamera(int i) {
        ...
        } else { SettingsManager.getInstance().setIsFrontCamera(true); }
    }
    ...
    private Camera.Parameters getParameters(int currentcamera) {
        ...
        switch (SettingsManager.getInstance().getFrameWork()) {
            case LG: {
                ...
                if (lgCamera == null || lgCamera.getCamera() == null) { lgCamera = new LGCameraRef(currentcamera); }
    ...
    private void detectVideoMediaProfiles(int cameraid) {
        ...
        } else { supportedProfiles = videoProfilesDetector.getDefaultVideoMediaProfiles(cameraid); }

        if (supportedProfiles.get(_720phfr) == null
            && SettingsManager.get(SettingKeys.VideoHighFramerate).isSupported()
            && SettingsManager.get(SettingKeys.VideoHighFramerate).contains("120")) {
            try {
        ...
        if (SettingsManager.get(SettingKeys.VideoSize).isSupported()
            && SettingsManager.get(SettingKeys.VideoSize)
            .contains("3840x2160")
            && SettingsManager.get(SettingKeys.VideoHighFramerate).isSupported()
            && SettingsManager.get(SettingKeys.VideoHighFramerate)
            .contains("60")) //<--- that line is not needed. when parameters contains empty hfr it gets filled!
        {
        ...
        if (supportedProfiles.get(_2160p) == null
            && SettingsManager.get(SettingKeys.VideoSize).isSupported()
            && SettingsManager.get(SettingKeys.VideoSize).contains("3840x2160")) {
            if (supportedProfiles.containsKey("1080p")) {
