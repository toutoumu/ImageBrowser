...
public class Camera2FeatureDetectorTask extends AbstractFeatureDetectorTask {
        ...
        if (hasCamera2Features) {
            ...
            for (int i = 0; i < parametersToDetect.size(); i++) {
                try {
                    ...
                    } else { parameter2Detector.checkIfSupported(characteristics); }
                } catch (NoSuchMethodException e) {
                    e.printStackTrace();
                } catch (IllegalAccessException e) {
                    e.printStackTrace();
    ...
    private void findCameraIds(CameraManager manager, List<String> cameraids) {
        ...
        for (int i = 0; i < 200; i++) {
            try {
                ...
                if (characteristics != null) {
                    ...
                    if (yuv && raw && jpeg && !logical) { nonlogicids.add(String.valueOf(i)); }
                }
            } catch (IllegalArgumentException ex) {
        ...
        if (nonlogicids.size() == 0) { cameraids.addAll(allcameraids); } else { cameraids.addAll(nonlogicids); }
    }
}
