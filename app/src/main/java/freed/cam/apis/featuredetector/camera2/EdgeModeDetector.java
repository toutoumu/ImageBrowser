...
public class EdgeModeDetector extends BaseParameter2Detector {
    ...
    protected void findAndFillSettings(CameraCharacteristics cameraCharacteristics) {
        Camera2Util.detectIntMode(cameraCharacteristics,
                                  ...
                                  SettingsManager.get(SettingKeys.EDGE_MODE),
                                  FreedApplication.getStringArrayFromRessource(R.array.edgeModes));
        ...
        for (int i = 0; i < vals.length; i++) {
            if (vals[i].contains(zsldnoise)) { containszslstring = true; }
        }
