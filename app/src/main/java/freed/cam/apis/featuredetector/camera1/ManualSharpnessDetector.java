...
public class ManualSharpnessDetector extends BaseParameter1Detector {
    ...
    private void detectManualSharpness(Camera.Parameters parameters) {
        if (SettingsManager.getInstance().getFrameWork() == Frameworks.MTK) {
            ...
            if (parameters.get(camstring(R.string.edge)) != null
                && parameters.get(camstring(R.string.edge_values)) != null) {
                SettingsManager.get(SettingKeys.M_Sharpness)
                    .setValues(parameters.get(camstring(R.string.edge_values)).split(","));
