    ...
    private void detectPreviewFpsRanges(Camera.Parameters parameters) {
        if (parameters.get(camstring(R.string.preview_fps_range_values)) != null) {
            try {
                ...
                SettingsManager.get(SettingKeys.PreviewFpsRange)
                    .setValues(parameters.get(camstring(R.string.preview_fps_range_values)).split(","));
                SettingsManager.get(SettingKeys.PreviewFpsRange)
                    .setCamera1ParameterKEY(camstring(R.string.preview_fps_range));
