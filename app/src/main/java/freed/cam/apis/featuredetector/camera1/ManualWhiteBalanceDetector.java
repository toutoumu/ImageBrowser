...
public class ManualWhiteBalanceDetector extends BaseParameter1Detector {
    ...
    private boolean arrayContainsString(String[] ar, String dif) {
        ...
        boolean ret = false;
        for (String s : ar)
            if (s.equals(dif)) { ret = true; }
        return ret;
    }

    private void detectManualWhiteBalance(Camera.Parameters parameters) {
        if (SettingsManager.get(SettingKeys.M_Whitebalance).isPresetted()) { return; }
        if (SettingsManager.getInstance().getFrameWork() == Frameworks.MTK) {
            SettingsManager.get(SettingKeys.M_Whitebalance).setIsSupported(false);
        } else if (SettingsManager.get(SettingKeys.M_Whitebalance)
            .isSupported()) // happens when its already set due supportedevices.xml
        { return; } else {
            ...
            if (arrayContainsString(SettingsManager.get(SettingKeys.WhiteBalanceMode).getValues(),
                                    FreedApplication.getStringFromRessources(R.string.manual))) {
                wbModeval = FreedApplication.getStringFromRessources(R.string.manual);
            } else if (arrayContainsString(SettingsManager.get(SettingKeys.WhiteBalanceMode).getValues(),
                                           FreedApplication.getStringFromRessources(R.string.manual_cct))) {
            ...
            try {
                if (!TextUtils.isEmpty(wbmax) && !TextUtils.isEmpty(wbmin) && !TextUtils.isEmpty(wbModeval)) {
                    ...
                    SettingsManager.get(SettingKeys.M_Whitebalance)
                        .setValues(Camera1FeatureDetectorTask.createWBStringArray(min, max, 100));
                } else {
