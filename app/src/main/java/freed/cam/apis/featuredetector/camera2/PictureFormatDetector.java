...
public class PictureFormatDetector extends BaseParameter2Detector {
    ...
    private void detectPictureFormats(CameraCharacteristics characteristics) {
        ...
        try {
            if (smap.isOutputSupportedFor(ImageFormat.NV16)) { Log.d(TAG, "Support NV16"); }
        } catch (IllegalArgumentException ex) {
            Log.d(TAG, "Dont support NV16");
        }
        try {
            if (smap.isOutputSupportedFor(ImageFormat.NV21)) { Log.d(TAG, "Support NV21"); }
        } catch (IllegalArgumentException e) {
            Log.d(TAG, "Dont support NV21");
        }
        try {
            if (smap.isOutputSupportedFor(ImageFormat.YUV_420_888)) { Log.d(TAG, "Support YUV_420_888"); }
        } catch (IllegalArgumentException e) {
            Log.d(TAG, "Dont support YUV_420_888");
        }
        try {
            if (smap.isOutputSupportedFor(ImageFormat.YUV_422_888)) { Log.d(TAG, "Support YUV_422_888"); }
        } catch (IllegalArgumentException e) {
            Log.d(TAG, "Dont support YUV_422_888");
        }
        try {
            if (smap.isOutputSupportedFor(ImageFormat.YUV_444_888)) { Log.d(TAG, "Support YUV_444_888"); }
        } catch (IllegalArgumentException e) {
            Log.d(TAG, "Dont support YUV_444_888");
        }
        try {
            if (smap.isOutputSupportedFor(ImageFormat.YV12)) { Log.d(TAG, "Support YV12"); }
        } catch (IllegalArgumentException e) {
            Log.d(TAG, "Dont support yv12");
        }
        try {
            if (smap.isOutputSupportedFor(ImageFormat.DEPTH16)) { Log.d(TAG, "Support DEPTH16"); }
        } catch (IllegalArgumentException e) {
            Log.d(TAG, "Dont support DEPTH16");
        }
        try {
            if (smap.isOutputSupportedFor(ImageFormat.DEPTH_POINT_CLOUD)) { Log.d(TAG, "Support DEPTH_POINT_CLOUD"); }
        } catch (IllegalArgumentException e) {
            Log.d(TAG, "Dont support DEPTH_POINT_CLOUD");
        }
        try {
            if (smap.isOutputSupportedFor(ImageFormat.RAW_PRIVATE)) { Log.d(TAG, "Support RAW_PRIVATE"); }
        } catch (IllegalArgumentException e) {
            Log.d(TAG, "Dont support RAW_PRIVATE");
        }
        try {
            if (smap.isOutputSupportedFor(ImageFormat.PRIVATE)) { Log.d(TAG, "Support PRIVATE"); }
        } catch (IllegalArgumentException e) {
        ...
        if (hmap.containsKey(FreedApplication.getStringFromRessources(R.string.pictureformat_jpeg))) {
            SettingsManager.get(SettingKeys.PictureFormat)
                .set(FreedApplication.getStringFromRessources(R.string.pictureformat_jpeg));
        } else if (hmap.containsKey(FreedApplication.getStringFromRessources(R.string.pictureformat_yuv))) {
            SettingsManager.get(SettingKeys.PictureFormat)
                .set(FreedApplication.getStringFromRessources(R.string.pictureformat_yuv));
        }
