...
public class ManualFocusDetector extends BaseParameter1Detector {
    ...
    private void detectManualFocus(Camera.Parameters parameters) {
        ...
        if (SettingsManager.get(SettingKeys.M_Focus).isPresetted()) { return; }
        ...
        if (SettingsManager.getInstance().getFrameWork() == Frameworks.MTK) {
            ...
            max = 1023;
            step = 10;
            SettingsManager.get(SettingKeys.M_Focus)
                .setCamera1ParameterKEY(FreedApplication.getStringFromRessources(R.string.afeng_pos));
            Log.d(TAG, "MF MTK");
        } else {
            //lookup old qcom
            ...
            try {
                if (parameters.get(camstring(R.string.manual_focus_modes)) == null) {
                    ...
                    if (parameters.get(camstring(R.string.max_focus_pos_index)) != null
                        ...
                        step = 10;
                        SettingsManager.get(SettingKeys.M_Focus)
                            .setCamera1ParameterKEY(camstring(R.string.manual_focus_position));
                ...
                //htc mf
                if (parameters.get(camstring(R.string.min_focus)) != null
                    && parameters.get(camstring(R.string.max_focus)) != null) {
                ...
                //huawai mf
                if (parameters.get(FreedApplication.getStringFromRessources(R.string.hw_vcm_end_value)) != null
                    && parameters.get(FreedApplication.getStringFromRessources(R.string.hw_vcm_start_value)) != null) {
                    Log.d(TAG, "Huawei MF");
                    SettingsManager.get(SettingKeys.M_Focus).setMode(camstring(R.string.manual));
                    SettingsManager.get(SettingKeys.M_Focus).setType(-1);
                    SettingsManager.get(SettingKeys.M_Focus).setIsSupported(true);
                    max =
                        Integer.parseInt(parameters.get(FreedApplication.getStringFromRessources(R.string.hw_vcm_end_value)));
                    min =
                        Integer.parseInt(parameters.get(FreedApplication.getStringFromRessources(R.string.hw_vcm_start_value)));
                    Log.d(TAG, "min/max mf:" + min + "/" + max);
                    step = 10;
                    SettingsManager.get(SettingKeys.M_Focus)
                        .setCamera1ParameterKEY(FreedApplication.getStringFromRessources(R.string.hw_manual_focus_step_value));
                }
        ...
        if (SettingsManager.get(SettingKeys.M_Focus).isSupported()) {
            SettingsManager.get(SettingKeys.M_Focus)
                .setValues(Camera1FeatureDetectorTask.createManualFocusValues(min, max, step));
        }
    }
}
