...
public class CameraFeatureDetector {
    ...
    public void detectFeatures() {
        ...
        if (SettingsManager.getInstance().hasCamera2Features()) {
            ...
            } else { SettingsManager.getInstance().setCamApi(SettingsManager.API_2); }
        }
        ...
        Log.d(TAG,
              "Feature Detection done! Start FreeDcam Api: "
                  + SettingsManager.getInstance().getCamApi()
                  + " app version:"
                  + SettingsManager.getInstance().getAppVersion());
        EventBusHelper.post(new SwichCameraFragmentEvent());
    }

    private void setGlobalDefaultSettings() {
        if (RenderScriptManager.isSupported()) {
            SettingsManager.getGlobal(SettingKeys.PREVIEW_POST_PROCESSING_MODE)
                .setValues(new String[] {
                    PreviewPostProcessingModes.off.name(), PreviewPostProcessingModes.RenderScript.name(),
                    PreviewPostProcessingModes.OpenGL.name()
                });
        ...
        } else { SettingsManager.getGlobal(SettingKeys.PREVIEW_POST_PROCESSING_MODE).setIsSupported(false); }

        SettingsManager.getGlobal(SettingKeys.GuideList)
            .setValues(FreedApplication.getContext().getResources().getStringArray(R.array.guidelist));
        SettingsManager.getGlobal(SettingKeys.GuideList)
            .set(SettingsManager.getGlobal(SettingKeys.GuideList).getValues()[0]);
        if (ReleaseChecker.isGithubRelease) { SettingsManager.getGlobal(SettingKeys.CHECKFORUPDATES).set(true); }
