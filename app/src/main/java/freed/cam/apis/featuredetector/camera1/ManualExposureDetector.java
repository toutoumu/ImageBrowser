...
public class ManualExposureDetector extends BaseParameter1Detector {
    ...
    private String[] getSupportedShutterValues(long minMillisec, long maxMiliisec, boolean withautomode) {
        ...
        if (withautomode) { tmp.add(FreedApplication.getStringFromRessources(R.string.auto_)); }
        for (int i = 0; i < allvalues.length; i++) {
            ...
            if (!s.equals(FreedApplication.getStringFromRessources(R.string.auto_))) {
                ...
                } else { a = Float.parseFloat(s) * 1000000f; }

                if (a >= minMillisec && a <= maxMiliisec) { tmp.add(s); }
    ...
    private void detectManualExposureTime(Camera.Parameters parameters) {
        ...
        if (SettingsManager.get(SettingKeys.M_ExposureTime).isPresetted()) { return; }
        //mtk shutter
        if (SettingsManager.getInstance().getFrameWork() == Frameworks.MTK) {
            ...
            SettingsManager.get(SettingKeys.M_ExposureTime)
                .setValues(FreedApplication.getContext().getResources().getStringArray(R.array.mtk_shutter));
            ...
            if (parameters.get(FreedApplication.getStringFromRessources(R.string.shutter)) != null) {
                ...
                SettingsManager.get(SettingKeys.M_ExposureTime).setIsSupported(true);
                SettingsManager.get(SettingKeys.M_ExposureTime)
                    .setValues(FreedApplication.getContext().getResources().getStringArray(R.array.htc));
                SettingsManager.get(SettingKeys.M_ExposureTime)
                    .setCamera1ParameterKEY(FreedApplication.getStringFromRessources(R.string.shutter));
            ...
            //lg shutter
                ...
                ArrayList<String> l =
                    new ArrayList(Arrays.asList(parameters.get(FreedApplication.getStringFromRessources(R.string.lg_shutterspeed_values))
                                                    .replace(",0", "")
                                                    .split(",")));
                l.remove(0);
                SettingsManager.get(SettingKeys.M_ExposureTime).setValues(l.toArray(new String[l.size()]));
                SettingsManager.get(SettingKeys.M_ExposureTime)
                    .setCamera1ParameterKEY(FreedApplication.getStringFromRessources(R.string.lg_shutterspeed));
                SettingsManager.get(SettingKeys.M_ExposureTime).setIsSupported(true);
            }
            //meizu shutter
                ...
                SettingsManager.get(SettingKeys.M_ExposureTime).setIsSupported(true);
                SettingsManager.get(SettingKeys.M_ExposureTime)
                    .setValues(FreedApplication.getContext().getResources().getStringArray(R.array.shutter_values_meizu));
            ...
            //kirin shutter
                ...
                try {
                    ...
                    if (split2[1].contains("/")) {
                        ...
                        max = (long) tmp;
                    } else { max = Long.parseLong(split[1]) * 1000000; }
            ...
            //qcom shutter
            else if (parameters.get(camstring(R.string.max_exposure_time)) != null
                && parameters.get(camstring(R.string.min_exposure_time)) != null) {
                long min = 0, max = 0;
                try {
                    ...
                    if (max > 0) {
                        ...
                        SettingsManager.get(SettingKeys.M_ExposureTime).setIsSupported(true);
                        SettingsManager.get(SettingKeys.M_ExposureTime)
                            .setCamera1ParameterKEY(camstring(R.string.exposure_time));
