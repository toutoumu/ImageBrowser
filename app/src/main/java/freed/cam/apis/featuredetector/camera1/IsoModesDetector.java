...
public class IsoModesDetector extends BaseParameter1Detector {
    ...
    private void detectIsoModes(Camera.Parameters parameters) {
        ...
        if (SettingsManager.get(SettingKeys.IsoMode).getValues() != null
            && SettingsManager.get(SettingKeys.IsoMode).getValues().length > 1) {
            SettingsManager.get(SettingKeys.IsoMode).setIsSupported(true);
        } else { SettingsManager.get(SettingKeys.IsoMode).setIsSupported(false); }
    }
}
