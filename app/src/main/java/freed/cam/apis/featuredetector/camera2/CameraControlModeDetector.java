    ...
    private void detectControlMode(CameraCharacteristics characteristics) {
        if (SettingsManager.getInstance().hasCamera2Features()) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                Camera2Util.detectIntMode(characteristics,
                                          CameraCharacteristics.CONTROL_AVAILABLE_MODES,
                                          SettingsManager.get(SettingKeys.CONTROL_MODE),
                                          FreedApplication.getStringArrayFromRessource(R.array.controlModes));
                return;
            } else {
                ...
                int[] full = null;
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M
                    && characteristics.get(CameraCharacteristics.CONTROL_AVAILABLE_MODES) != null) {
                    full = characteristics.get(CameraCharacteristics.CONTROL_AVAILABLE_MODES);
                } else if (device == CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL_FULL
                    || device == CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL_3
                    || device == CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL_LIMITED) {
                    full = new int[] { 0, 1, 2, };
                ...
                if (SettingsManager.get(SettingKeys.CONTROL_MODE).isSupported()) {
                    ...
                    SettingsManager.get(SettingKeys.CONTROL_MODE)
                        .set(FreedApplication.getStringFromRessources(R.string.auto));
                }
            }
        }
    }
