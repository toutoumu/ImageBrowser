    ...
    private void detectVideoHdr(Camera.Parameters parameters) {
        if (parameters.get(camstring(R.string.video_hdr_values)) != null) {
            ...
            detectMode(parameters,
                       R.string.sony_video_hdr,
                       R.string.sony_video_hdr_values,
                       SettingsManager.get(SettingKeys.VideoHDR));
        } else { SettingsManager.get(SettingKeys.VideoHDR).setIsSupported(false); }
    }
}
