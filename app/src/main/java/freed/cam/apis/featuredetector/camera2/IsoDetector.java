...
public class IsoDetector extends BaseParameter2Detector {
    ...
    private void detectManualIso(CameraCharacteristics characteristics) {
        ...
        if (SettingsManager.getInstance().getCamera2MaxIso() > 0) { max = SettingsManager.getInstance().getCamera2MaxIso(); }
        ...
        if (ar.size() > 0) { SettingsManager.get(SettingKeys.M_ManualIso).setValues(ar.toArray(new String[ar.size()])); }
    }
