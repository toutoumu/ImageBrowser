...
public class Camera2Util {
    ...
    public static void detectIntMode(CameraCharacteristics characteristics,
                                     CameraCharacteristics.Key<int[]> requestKey,
        ...
        if (SettingsManager.getInstance().hasCamera2Features() && characteristics.get(requestKey) != null) {
            ...
            if (scenes.length > 1) { settingMode.setIsSupported(true); } else {
            ...
            for (int i = 0; i < scenes.length; i++) {
                int t = scenes[i];
                if (t < lookupar.length) { map.put(lookupar[t], t); }
            }
    ...
    public static void detectByteMode(CameraCharacteristics characteristics,
                                      CameraCharacteristics.Key<byte[]> requestKey,
                                      SettingMode settingMode,
                                      String[] lookupar) {
        if (SettingsManager.getInstance().hasCamera2Features() && characteristics.get(requestKey) != null) {
    ...
    public static void dumpIntArray(int[] array, String TAG, String name) {
        if (array != null) { Log.d(TAG, name + ":" + Arrays.toString(array)); } else { Log.d(TAG, name + ": isNull"); }
    }
}
