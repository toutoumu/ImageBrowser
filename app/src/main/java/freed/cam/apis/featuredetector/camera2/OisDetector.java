...
public class OisDetector extends BaseParameter2Detector {
    ...
    protected void findAndFillSettings(CameraCharacteristics characteristics) {
        ...
        if (oisvalues.length > 1) {
            if (oisvalues[0] == 1) { ois_supported = true; }
            if (oisvalues[1] == 1) { ois_supported = true; }
        } else if (oisvalues.length == 1) { if (oisvalues[0] == 1) { ois_supported = true; } }
        ...
        } else { SettingsManager.get(SettingKeys.OIS_MODE).setIsSupported(false); }
    }
}
