...
public class ProgramShiftManualSony extends BaseManualParameterSony {
    ...
    public ProgramShiftManualSony(CameraWrapperInterface cameraUiWrapper) {
        ...
        BaseManualParameterSony fnumber =
            (BaseManualParameterSony) cameraUiWrapper.getParameterHandler().get(SettingKeys.M_Fnumber);
    }
    ...
    public String[] getStringValues() {
        if (stringvalues == null || stringvalues.length == 0) { getminmax(); }
        return stringvalues;
    }

    @Override
    public String GetStringValue() {
        if (stringvalues == null || stringvalues.length < currentInt) { getminmax(); }
        if (stringvalues != null && stringvalues.length > currentInt) { return stringvalues[currentInt]; }
        return "0";
    }
    ...
    private void getminmax() {
        FreeDPool.Execute(() -> {
            try {
                ...
                JSONObject object =
                    ((ParameterHandler) cameraUiWrapper.getParameterHandler()).mRemoteApi.getParameterFromCamera(
                        VALUES_TO_GET);
                JSONArray array = object.getJSONArray("result");
                JSONArray subarray = array.getJSONArray(0);
                stringvalues = JsonUtils.ConvertJSONArrayToStringArray(subarray);
                if (stringvalues == null || stringvalues.length != 2) { return; }
