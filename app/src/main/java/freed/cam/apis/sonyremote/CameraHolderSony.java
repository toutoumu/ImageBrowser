...
public class CameraHolderSony extends CameraHolderAbstract implements CameraHolderSonyInterface {
    ...
    public CameraHolderSony(Context context,
                            PreviewStreamDrawer simpleStreamSurfaceView,
    ...
    public void StartPreview() {
        try {
            ...
            } else { replyJson = mRemoteApi.startLiveview(); }
            ...
            } else { Log.d(TAG, "Error : " + replyJson); }
        } catch (IOException e) {
    ...
    private void closeConnection() {
        ...
        if (mLiveviewSurface != null) {
            if (serverDevice != null
                && (serverDevice.getFriendlyName().contains("ILCE-QX1") || serverDevice.getFriendlyName()
                .contains("ILCE-QX30"))
                && JsonUtils.isApiSupported("setLiveviewFrameInfo",
                                            ((SonyCameraRemoteFragment) cameraUiWrapper).getAvailableApiSet())) {
        ...
        if (JsonUtils.isCameraApiAvailable("stopRecMode",
                                           ((SonyCameraRemoteFragment) cameraUiWrapper).getAvailableApiSet())) {
    ...
    public void TakePicture(I_PictureCallback pictureCallback) {
        ImageManager.putImageLoadTask(new ActTakePictureRunner(mRemoteApi,
                                                               pictureCallback,
    ...
    public boolean canCancelFocus() {
        if (((SonyCameraRemoteFragment) cameraUiWrapper).getAvailableApiSet().contains("cancelTouchAFPosition")
            || ((SonyCameraRemoteFragment) cameraUiWrapper).getAvailableApiSet().contains("cancelTrackingFocus")) {
