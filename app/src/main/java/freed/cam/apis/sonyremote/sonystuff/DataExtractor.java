...
public class DataExtractor {
    ...
    private void readData(InputStream mInputStream) throws IOException {
        if (commonHeader.PayloadType == 1) { jpegData = SimpleLiveviewSlicer.readBytes(mInputStream, jpegSize); }
        ...
        if (commonHeader.PayloadType == 2) {
            ...
            for (int i = 0; i < frameCount; i++) {
                ...
                if (framebytes.length == singleFrameDataSize) { frameInfoList.add(new FrameInfo(framebytes)); }
            }
        }
    }
    ...
    public class PayLoadHeader {
        public PayLoadHeader(byte[] bytes) throws IOException {
            ...
            if (commonHeader.PayloadType == 1) { jpegSize = SimpleLiveviewSlicer.bytesToInt(bytes, 4, 3); }
            ...
            if (commonHeader.PayloadType == 2) {
                ...
                if (version.equals("1.0")) { singleFrameDataSize = 16; } else {
