...
public class JoyPad extends View implements ParameterEvents {
    ...
    public void onIntValueChanged(int current) {
        if (current > 1) { this.setVisibility(VISIBLE); } else { this.setVisibility(GONE); }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                int x = 0, y = 0;
                if (joypad_posX > getWidth() / 2) { x = 1; } else if (joypad_posX < getWidth() / 2) { x = -1; }
                if (joypad_posY > getHeight() / 2) { y = 1; } else if (joypad_posX < getHeight() / 2) { y = -1; }
