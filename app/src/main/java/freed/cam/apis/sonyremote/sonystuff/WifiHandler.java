...
public class WifiHandler extends WifiUtils {
    ...
    private void sendMessage(String msg) {
        if (eventsListner != null) { eventsListner.onMessage(msg); }
    }

    private void sendDevice(ServerDevice device) {
        if (eventsListner != null) { eventsListner.onDeviceFound(device); }
    }
    ...
    public void StartLookUp() {
        ...
        if (isWifiEnabled() && isLocationServiceEnabled()) {
            ...
            if (!isWifiEnabled()) { sendMessage("Pls enable Wifi"); }
            if (!isLocationServiceEnabled()) { sendMessage("Pls enable LocationService"); }
            postDelayed(500);
        }
    }
