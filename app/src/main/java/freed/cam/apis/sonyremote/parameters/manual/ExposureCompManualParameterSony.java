...
public class ExposureCompManualParameterSony extends BaseManualParameterSony {
    ...
    public ExposureCompManualParameterSony(CameraWrapperInterface cameraUiWrapper) {
        super("getExposureCompensation",
              "getAvailableExposureCompensation",
              "setExposureCompensation",
              cameraUiWrapper,
              SettingKeys.M_ExposureCompensation);
        currentInt = -200;
    }
    ...
    public void SetValue(final int valueToSet, boolean setToCamera) {
        ...
        FreeDPool.Execute(() -> {
            ...
            try {
                ...
                if (valueToSet > stringvalues.length) { toset = stringvalues.length - 1; } else { toset = valueToSet; }
                array = new JSONArray().put(0, Integer.parseInt(stringvalues[toset]));
                JSONObject object =
                    ((ParameterHandler) cameraUiWrapper.getParameterHandler()).mRemoteApi.setParameterToCamera(VALUE_TO_SET,
                                                                                                               array);
    ...
    public String[] getStringValues() {
        if (stringvalues == null || stringvalues.length == 0) { getMinMaxValues(); }
        return stringvalues;
    }
