 ...
 */
public class ParameterHandler extends AbstractParameterHandler<SonyRemoteCamera>
    implements SimpleCameraEventObserver.ChangeListener {
    ...
    public ParameterHandler(SonyRemoteCamera cameraUiWrapper, PreviewStreamDrawer surfaceView) {
        ...
        if (RenderScriptManager.isSupported()) {
            SettingsManager.getGlobal(SettingKeys.FOCUSPEAK_COLOR)
                .setValues(FreedApplication.getContext().getResources().getStringArray(R.array.focuspeakColors));
            SettingsManager.getGlobal(SettingKeys.FOCUSPEAK_COLOR)
                .set(SettingsManager.getGlobal(SettingKeys.FOCUSPEAK_COLOR).getValues()[0]);
    ...
    private void throwSonyApiChanged(Set<String> mAvailableCameraApiSet) {
        for (int i = 0; i < parametersChangedList.size(); i++) {
            if (parametersChangedList.get(i) == null) {
                ...
                i--;
            } else { parametersChangedList.get(i).SonyApiChanged(mAvailableCameraApiSet); }
        }
    }
    ...
    private void createParameters() {
        ...
        add(SettingKeys.FlashMode,
            new BaseModeParameterSony("getFlashMode",
                                      "setFlashMode",
                                      "getAvailableFlashMode",
                                      mRemoteApi,
                                      cameraUiWrapper,
                                      SettingKeys.FlashMode));
        parametersChangedList.add((BaseModeParameterSony) get(SettingKeys.FlashMode));

        add(SettingKeys.ExposureMode,
            new BaseModeParameterSony("getExposureMode",
                                      "setExposureMode",
                                      "getAvailableExposureMode",
                                      mRemoteApi,
                                      cameraUiWrapper,
                                      SettingKeys.ExposureMode));
        parametersChangedList.add((BaseModeParameterSony) get(SettingKeys.ExposureMode));

        add(SettingKeys.ContShootMode,
            new ContShootModeParameterSony(mRemoteApi, cameraUiWrapper.getModuleHandler(), cameraUiWrapper));
        parametersChangedList.add((BaseModeParameterSony) get(SettingKeys.ContShootMode));

        add(SettingKeys.ContShootModeSpeed,
            new BaseModeParameterSony("getContShootingSpeed",
                                      "setContShootingSpeed",
                                      "getAvailableContShootingSpeed",
                                      mRemoteApi,
                                      cameraUiWrapper,
                                      SettingKeys.ContShootModeSpeed));
        parametersChangedList.add((BaseModeParameterSony) get(SettingKeys.ContShootModeSpeed));

        add(SettingKeys.FocusMode,
            new FocusModeSony("getFocusMode", "setFocusMode", "getAvailableFocusMode", mRemoteApi, cameraUiWrapper));
        ...
        add(SettingKeys.M_ExposureTime,
            new BaseManualParameterSony("getShutterSpeed",
                                        "getAvailableShutterSpeed",
                                        "setShutterSpeed",
                                        cameraUiWrapper,
                                        SettingKeys.M_ExposureTime));
        parametersChangedList.add((BaseManualParameterSony) get(SettingKeys.M_ExposureTime));

        add(SettingKeys.M_Fnumber,
            new BaseManualParameterSony("getFNumber",
                                        "getAvailableFNumber",
                                        "setFNumber",
                                        cameraUiWrapper,
                                        SettingKeys.M_Fnumber));
        parametersChangedList.add((BaseManualParameterSony) get(SettingKeys.M_Fnumber));

        add(SettingKeys.M_ManualIso,
            new BaseManualParameterSony("getIsoSpeedRate",
                                        "getAvailableIsoSpeedRate",
                                        "setIsoSpeedRate",
                                        cameraUiWrapper,
                                        SettingKeys.M_ManualIso));
        ...
        add(SettingKeys.WhiteBalanceMode,
            new WhiteBalanceModeSony(mRemoteApi, (WbCTManualSony) get(SettingKeys.M_Whitebalance), cameraUiWrapper));
        parametersChangedList.add((BaseModeParameterSony) get(SettingKeys.WhiteBalanceMode));

        add(SettingKeys.PostViewSize,
            new BaseModeParameterSony("getPostviewImageSize",
                                      "setPostviewImageSize",
                                      "getAvailablePostviewImageSize",
                                      mRemoteApi,
                                      cameraUiWrapper,
                                      SettingKeys.PostViewSize));
        parametersChangedList.add((BaseModeParameterSony) get(SettingKeys.PostViewSize));

        add(SettingKeys.VideoSize,
            new BaseModeParameterSony("getMovieQuality",
                                      "setMovieQuality",
                                      "getAvailableMovieQuality",
                                      mRemoteApi,
                                      cameraUiWrapper,
                                      SettingKeys.VideoSize));
    ...
    public void onShootModeChanged(String shootMode) {
        if (cameraShotMode != null) { cameraShotMode.onShootModeChanged(shootMode); }
    }
    ...
    public void onCameraStatusChanged(String status) {
        ...
        if (CameraStatusListner != null) { CameraStatusListner.onCameraStatusChanged(status); }
    }
    ...
    public void onWhiteBalanceValueChanged(String wb) {
        ...
        if (get(SettingKeys.WhiteBalanceMode).GetStringValue().equals("Color Temperature")
            && get(SettingKeys.M_Whitebalance) != null) {
            get(SettingKeys.M_Whitebalance).setViewState(AbstractParameter.ViewState.Visible);
        } else { get(SettingKeys.M_Whitebalance).setViewState(AbstractParameter.ViewState.Hidden); }
    }
        ...
        FreeDPool.Execute(() -> {
            for (String s : url) {
                if (cameraUiWrapper.getModuleHandler().getCurrentModule() instanceof PictureModuleSony) {
                    PictureModuleSony pictureModuleSony =
                        (PictureModuleSony) cameraUiWrapper.getModuleHandler().getCurrentModule();
                    try {
    ...
    public void onExposureModeChanged(String expomode) {
        ...
        } else { get(SettingKeys.WhiteBalanceMode).setViewState(AbstractParameter.ViewState.Visible); }
    }
}
