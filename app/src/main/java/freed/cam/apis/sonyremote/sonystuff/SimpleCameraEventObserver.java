...
public class SimpleCameraEventObserver {
    ...
    private void sendLog(String msg) {
        ...
        if (LOGGING) { Log.d(SimpleCameraEventObserver.TAG, msg); }
    }
    ...
    public void start() {
        ...
        FreeDPool.Execute(() -> {
            ...
            while (mWhileEventMonitoring) {
                ...
                try {
                    // Call getEvent API.
                    replyJson = SimpleCameraEventObserver.this.mRemoteApi.getEvent(longPolling,
                                                                                   SimpleCameraEventObserver.this.version);
                    if (firstCall) { Log.d(TAG, replyJson.toString()); }
                    ...
                    try {
                        replyJson = SimpleCameraEventObserver.this.mRemoteApi.getEvent(false,
                                                                                       SimpleCameraEventObserver.this.version);
                        SimpleCameraEventObserver.this.processEvents(replyJson);
    ...
    public void processEvents(JSONObject replyJson) throws JSONException {
        ...
        String[] postviews =
            JsonUtils.findStringArrayInformation(replyJson, 19, "postviewImageSize", "postviewImageSizeCandidates");
        ...
        if (touchSuccess != null || !TextUtils.isEmpty(touchSuccess)) { this.sendLog("got focus sucess:" + touchSuccess); }
        ...
        if (!TextUtils.isEmpty(focusStatus)) {
            ...
            if (focusStatus.equals("Not Focusing")) { this.fireFocusLockedChangeListener(false); }
            if (focusStatus.equals("Focused")) { this.fireFocusLockedChangeListener(true); }
        }
        ...
        String trackingFocusStatus =
            JsonUtils.findStringInformation(replyJson, 54, "trackingFocusStatus", "trackingFocusStatus");
        if (!TextUtils.isEmpty(trackingFocusStatus)) {
            this.sendLog("tracking focusstate: " + trackingFocusStatus);
            if (trackingFocusStatus.equals("Tracking")) { this.fireFocusLockedChangeListener(true); }
            if (trackingFocusStatus.equals("Not Tracking")) { this.fireFocusLockedChangeListener(false); }
        }
