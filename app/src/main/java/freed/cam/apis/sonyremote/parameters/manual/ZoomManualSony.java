...
public class ZoomManualSony extends BaseManualParameterSony {
    ...
    public void SonyApiChanged(Set<String> mAvailableCameraApiSet) {
        ...
        if (JsonUtils.isCameraApiAvailable("actZoom", mAvailableCameraApiSet)) { setViewState(ViewState.Visible); }
        stringvalues = createStringArray(0, 100, 1);
    }

    @Override
    public ViewState getViewState() {
        if (((ParameterHandler) cameraUiWrapper.getParameterHandler()).mAvailableCameraApiSet != null
            && JsonUtils.isCameraApiAvailable("actZoom",
                                              ((ParameterHandler) cameraUiWrapper.getParameterHandler()).mAvailableCameraApiSet)) {
            return ViewState.Visible;
        } else { return ViewState.Hidden; }
    }
    ...
    public void SetValue(int valueToSet, boolean setToCamera) {
        ...
        if (!isZooming) {
            ...
            String direction;
            if (valueToSet < currentInt) { direction = "out"; } else { direction = "in"; }
    ...
    public void setZoomsHasChanged(int zoom) {
        ...
        if (zoomToSet != currentInt && fromUser) {
            if (!checkIfIntIsInRange(zoomToSet, currentInt)) { SetValue(zoomToSet, true); } else {
                zoomToSet = currentInt;
                fromUser = false;
            }
        } else { zoomToSet = currentInt; }
