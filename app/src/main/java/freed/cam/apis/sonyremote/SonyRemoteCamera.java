...
public class SonyRemoteCamera extends AbstractCamera<ParameterHandler, CameraHolderSony, ModuleHandlerSony>
    ...
    public void stopCamera() {
        if (mEventObserver != null) { mEventObserver.stop(); }
        ...
        try {
           ...
           /* replyJson = mRemoteApi.getEvent(false, "1.0");
            ...
            if (!JsonUtils.isApiSupported("setCameraFunction", mAvailableCameraApiSet) &&
                !(JsonUtils.isApiSupported("startContShooting", mAvailableCameraApiSet) && JsonUtils.isApiSupported(
                    "stopContShooting",
                    mAvailableCameraApiSet))) {
    ...
    private void openConnection() {
        ...
        try {
            ...
            if (!mEventObserver.isActive()) { mEventObserver.activate(); }
            ...
            if (JsonUtils.isCameraApiAvailable("getEvent", mAvailableCameraApiSet)) {
                ...
                if (!mEventObserver.isStarted()) { mEventObserver.start(); }
            }
    ...
    public void release() {
        if (mEventObserver != null) { mEventObserver.release(); }
    }
}
