...
public class WbCTManualSony extends BaseManualParameterSony {
    ...
    public void SetValue(int valueToSet, boolean setToCamera) {
        ...
        if (valueToSet > values.length) { currentInt = values.length; }
        if (valueToSet < 0) { currentInt = 0; }
        ...
        FreeDPool.Execute(() -> {
            try {
                ...
                if (valueToSet == 0) { array = new JSONArray().put("Auto WB").put(false); } else {
    ...
    public void setValueInternal(int val) {
        for (int i = 0; i < values.length; i++) {
            if (values[i].equals(val)) { currentInt = i; }
        }
        if (currentInt == -200) { return; }
