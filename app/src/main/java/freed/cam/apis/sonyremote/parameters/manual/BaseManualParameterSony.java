...
public class BaseManualParameterSony extends AbstractParameter implements I_SonyApi, ParameterEvents {
    ...
    public BaseManualParameterSony(String VALUE_TO_GET,
    ...
    public void SonyApiChanged(Set<String> mAvailableCameraApiSet) {
        ...
        } else if (isSupported && !isSetSupported) { setViewState(ViewState.Disabled); } else {
            setViewState(ViewState.Hidden);
        }
        Log.d(TAG,
              "SonyApiCHanged setViewState for "
                  + VALUE_TO_GET
                  + " isDisabled: "
                  + isSupported
                  + " isVisible: "
    ...
    public void SetValue(final int valueToSet, boolean setToCamera) {
        ...
        FreeDPool.Execute(() -> {
            if (valueToSet >= stringvalues.length || valueToSet < 0) { return; }
    ...
    public String GetStringValue() {
        ...
        if (value == null || TextUtils.isEmpty(value)) {
            ...
            if (stringvalues != null && stringvalues.length > 0 && currentInt < stringvalues.length) {
                if (currentInt == -200) { GetValue(); }
                if (currentInt == -1) { return value; }
    ...
    public void onStringValueChanged(String value) {
        ...
        for (int i = 0; i < stringvalues.length; i++) {
            if (value.equals(stringvalues[i])) { onIntValueChanged(i); }
        }
    }

    protected void sendLog(String log) {
        if (logging) { Log.d(TAG, VALUE_TO_SET + ":" + log); }
    }
}
