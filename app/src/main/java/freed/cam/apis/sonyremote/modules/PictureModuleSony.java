...
public class PictureModuleSony extends ModuleAbstract implements I_PictureCallback, I_CameraStatusChanged {
    ...
    public void DoWork() {
        if (((ParameterHandler) cameraUiWrapper.getParameterHandler()).canStartBulbCapture()) {
            ...
            && cameraUiWrapper.getParameterHandler().get(SettingKeys.ContShootMode).getViewState()
    ...
    public void InitModule() {
        ...
        } else { changeCaptureState(CaptureStates.image_capture_stop); }
    ...
    public void onPictureTaken(URL url) {
        File file = new File(cameraUiWrapper.getActivityInterface()
                                 .getFileListController()
                                 .getNewFilePath(SettingsManager.getInstance().GetWriteExternal(), ".jpg"));
        try {
            file.createNewFile();
        ...
        try {
            ...
            if (VERSION.SDK_INT <= VERSION_CODES.LOLLIPOP
                || VERSION.SDK_INT >= VERSION_CODES.LOLLIPOP && !SettingsManager.getInstance().GetWriteExternal()) {
