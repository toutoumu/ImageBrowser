...
public class RawImageCapture extends StillImageCapture {
    ...
    public RawImageCapture(Size size,
                           int format,
    ...
    protected static ImageTask process_rawWithDngConverter(byte[] bytes,
        ...
        try {
            saveTask.setExposureIndex(captureResult.get(CaptureResult.CONTROL_AE_EXPOSURE_COMPENSATION)
        ...
        if (SettingsManager.get(SettingKeys.useCustomMatrixOnCamera2).get()
            && SettingsManager.getInstance().getDngProfilesMap().get(bytes.length) != null) {
    ...
    protected ImageTask process_rawSensor(Image image, File file, CaptureResult captureResult) {
        ImageTaskDngConverter taskDngConverter = new ImageTaskDngConverter(captureResult,
                                                                           image,
                                                                           characteristics,
                                                                           file,
                                                                           activityInterface,
