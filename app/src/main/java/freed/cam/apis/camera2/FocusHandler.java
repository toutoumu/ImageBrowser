...
public class FocusHandler extends AbstractFocusHandler<Camera2> implements EventBusLifeCycle {
    ...
    public void onFocusModeValueChanged(ValueChangedEvent<String> valueChangedEvent) {
        if (valueChangedEvent.type != String.class || cameraUiWrapper == null) { return; }
        if (valueChangedEvent.key == SettingKeys.FocusMode) {
            ...
            if (val.contains("Continous") || val.equals(FreedApplication.getStringFromRessources(R.string.off))) {
                focusenabled = false;
                if (focusEvent != null) { focusEvent.TouchToFocusSupported(false); }
            } else {
                focusenabled = true;
                if (focusEvent != null) { focusEvent.TouchToFocusSupported(true); }
            }
        }
    }
    ...
    public void StartTouchToFocus(int x, int y, int width, int height) {
        ...
        if (focusEvent != null) { focusEvent.FocusStarted(x, y); }
    }

    @Override
    protected void startTouchFocus(FocusCoordinates viewCoordinates) {
        //logFocusRect(rect);
        Log.d(TAG,
              "Width:"
                  + viewCoordinates.width
                  + "Height"
                  + viewCoordinates.height
                  + " X: "
                  + viewCoordinates.x
                  + "Y : "
                  + viewCoordinates.y);
        ...
        Rect sensorSize =
            ((CameraHolderApi2) cameraUiWrapper.getCameraHolder()).characteristics.get(CameraCharacteristics.SENSOR_INFO_ACTIVE_ARRAY_SIZE);
        logRect(sensorSize);
        ...
        logFocusRect(targetFocusRect);
        MeteringRectangle rectangle = new MeteringRectangle(targetFocusRect.left,
                                                            targetFocusRect.top,
                                                            targetFocusRect.right,
                                                            targetFocusRect.bottom,
                                                            1000);
        MeteringRectangle[] mre = { rectangle };
        cameraUiWrapper.captureSessionHandler.SetFocusArea(CaptureRequest.CONTROL_AF_REGIONS, mre);

        // FIXME: 2018/3/23 Camera2 设置白平衡中心区域
        cameraUiWrapper.captureSessionHandler.SetParameter(CaptureRequest.CONTROL_AE_REGIONS, mre);
        cameraUiWrapper.captureSessionHandler.SetParameter(CaptureRequest.CONTROL_AE_PRECAPTURE_TRIGGER,
    ...
    public void SetMeteringAreas(int x, int y, int width, int height) {
        ...
        if (targetFocusRect.left < 0) { targetFocusRect.left = sensor_size.left; }
        if (targetFocusRect.right > sensor_size.right) { targetFocusRect.right = sensor_size.right; }
        if (targetFocusRect.top < 0) { targetFocusRect.top = sensor_size.top; }
        if (targetFocusRect.bottom > sensor_size.bottom) { targetFocusRect.bottom = sensor_size.bottom; }
