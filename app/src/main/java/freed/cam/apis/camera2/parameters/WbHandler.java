...
public class WbHandler {
    ...
    public WbHandler(Camera2 cameraUiWrapper) {
        ...
        if (SettingsManager.get(SettingKeys.WhiteBalanceMode).isSupported()) { whiteBalanceApi2 = new WhiteBalanceApi2(); }
    ...
    private void setWbMode(String wbMode) {
        if (!wbMode.equals(FreedApplication.getStringFromRessources(R.string.off))) {
            ...
            if (manualWbCt != null) { manualWbCt.setViewState(AbstractParameter.ViewState.Hidden); }
        } else //if OFF
        {
            //set colorcorrection to TRANSFORMATRIX to have full control
            colorCorrectionMode.SetValue(FreedApplication.getStringFromRessources(R.string.colorcorrection_transform_matrix),
                                         true);
    ...
    public class WhiteBalanceApi2 extends BaseModeApi2 {
        ...
        public WhiteBalanceApi2() {
            ...
            parameterValues =
                StringUtils.StringArrayToIntHashmap(SettingsManager.get(SettingKeys.WhiteBalanceMode).getValues());
    ...
    public class ManualWbCtApi2 extends AbstractParameter<Camera2> {
        ...
        public ManualWbCtApi2(Camera2 cameraUiWrapper) {
            ...
            lookupvalues =
                new StringIntArray(FreedApplication.getContext().getResources().getStringArray(R.array.wbct_lookup));
            currentInt = 0;
        }
        ...
        public void setValue(int valueToSet, boolean setToCamera) {
            ...
            cameraUiWrapper.captureSessionHandler.SetParameterRepeating(CaptureRequest.COLOR_CORRECTION_GAINS,
                                                                        wbChannelVector,
                                                                        setToCamera);
        }

        private int checkminmax(int val) {
            if (val > 255) { return 255; } else if (val < 0) { return 0; } else return val;
        }

        private double getRGBToDouble(int color) {
            double s = ((2.0 / 255.0) * color) + 1;
            if (s < MINCAP) { return MINCAP; }
            return s;
        }
        ...
        public ViewState getViewState() {
            try {
                if (cameraUiWrapper == null
                    || cameraUiWrapper.getParameterHandler() == null
                    || cameraUiWrapper.getParameterHandler().get(SettingKeys.WhiteBalanceMode) == null) {
                    return ViewState.Hidden;
                } else if (cameraUiWrapper.getParameterHandler()
                    .get(SettingKeys.WhiteBalanceMode)
                    .GetStringValue()
                    .equals(FreedApplication.getStringFromRessources(R.string.off))) { return ViewState.Visible; }
    ...
    public class ColorCorrectionModeApi2 extends BaseModeApi2 {
        ...
        public ColorCorrectionModeApi2() {
            ...
            if (SettingsManager.get(SettingKeys.COLOR_CORRECTION_MODE).isSupported()) { setViewState(ViewState.Visible); }
        }

        @Override
        public void setValue(String valueToSet, boolean setToCamera) {
            cameraUiWrapper.captureSessionHandler.SetParameterRepeating(CaptureRequest.COLOR_CORRECTION_MODE,
                                                                        parameterValues.get(valueToSet),
                                                                        setToCamera);
            cameraUiWrapper.captureSessionHandler.SetParameterRepeating(CaptureRequest.COLOR_CORRECTION_GAINS,
                                                                        null,
                                                                        setToCamera);
