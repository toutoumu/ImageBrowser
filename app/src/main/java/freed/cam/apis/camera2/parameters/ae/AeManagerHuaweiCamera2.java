...
public class AeManagerHuaweiCamera2 extends AeManagerCamera2 {
    ...
    public boolean isExposureTimeWriteable() {
        return activeAeState == AeStates.shutter_priority
            || activeAeState == AeStates.manual
            || activeAeState == AeStates.auto;
    }

    @Override
    public void setExposureTime(int valueToSet, boolean setToCamera) {
        cameraUiWrapper.captureSessionHandler.SetParameterRepeating(CaptureRequestHuawei.HUAWEI_PROFESSIONAL_MODE,
                                                                    CaptureRequestHuawei.HUAWEI_PROFESSIONAL_MODE_ENABLED,
                                                                    setToCamera);
        if (valueToSet > 0) {
            int val =
                (int) AbstractManualShutter.getMilliSecondStringFromShutterString(manualExposureTime.getStringValues()[valueToSet]);
            cameraUiWrapper.captureSessionHandler.SetParameterRepeating(CaptureRequestHuawei.HUAWEI_SENSOR_EXPOSURE_TIME,
                                                                        val,
                                                                        setToCamera);
    ...
    private void applyAeMode() {
        if (expotimeIsActive && isoIsActive) { setAeMode(AeStates.manual); } else if (!expotimeIsActive && isoIsActive) {
            setAeMode(AeStates.iso_priority);
        } else if (expotimeIsActive
            && !isoIsActive) { setAeMode(AeStates.shutter_priority); } else if (!expotimeIsActive
            && !isoIsActive) { setAeMode(AeStates.auto); }
    }
    ...
    public void setIso(int valueToSet, boolean setToCamera) {
        cameraUiWrapper.captureSessionHandler.SetParameterRepeating(CaptureRequestHuawei.HUAWEI_PROFESSIONAL_MODE,
                                                                    ...
                                                                    setToCamera);
        if (valueToSet == 0) {
            cameraUiWrapper.captureSessionHandler.SetParameterRepeating(CaptureRequestHuawei.HUAWEI_SENSOR_ISO_VALUE,
                                                                        0,
                                                                        setToCamera);
            isoIsActive = false;
        } else {
    ...
    public void setExposureCompensation(int valueToSet, boolean setToCamera) {
        ...
        cameraUiWrapper.captureSessionHandler.SetParameterRepeating(CaptureRequestHuawei.HUAWEI_EXPOSURE_COMP_VALUE,
                                                                    t,
                                                                    setToCamera);
    }
    ...
    public void setAeMode(AeStates aeState) {
        if (activeAeState == aeState) { return; }
