...
public class AeBracketApi2 extends PictureModuleApi2 {
    ...
    protected void prepareCaptureBuilder(int captureNum) {
        ...
        cameraUiWrapper.captureSessionHandler.SetCaptureParameter(CaptureRequest.CONTROL_AE_MODE,
                                                                  CaptureRequest.CONTROL_AE_MODE_OFF);

        if (currentiso >= maxiso) { currentiso = maxiso; }
        ...
        if (0 == captureNum) { expotimeToSet = currentExposureTime - exposureTimeStep; } else if (1 == captureNum) {
        ...
        Log.d(TAG,
              "request: "
                  + captureNum
                  + " AE Mode:"
                  + cameraUiWrapper.captureSessionHandler.getImageCaptureParameter(CaptureRequest.CONTROL_AE_MODE)
                  + " Expotime:"
                  + cameraUiWrapper.captureSessionHandler.getImageCaptureParameter(CaptureRequest.SENSOR_EXPOSURE_TIME)
                  + " iso:"
                  + cameraUiWrapper.captureSessionHandler.getImageCaptureParameter(CaptureRequest.SENSOR_SENSITIVITY));
    }
    ...
    protected void finishCapture() {
        ...
        if (BurstCounter.getImageCaptured() == 3) {
            if (aeWasOn && parameterHandler.get(SettingKeys.ExposureMode) != null) {
                parameterHandler.get(SettingKeys.ExposureMode)
                    .SetValue(FreedApplication.getContext().getString(R.string.on), true);
            }
        }
    }
}
