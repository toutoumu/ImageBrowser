...
public class CameraValuesChangedCaptureCallback extends CameraCaptureSession.CaptureCallback implements HistogramFeed {
    ...
    public void setWaitForAe_af_lock(WaitForAe_Af_Lock callback) {
        if (callback != null) {
            ...
            focusState = WAITFORSCAN;
        } else { Log.d(TAG, "clear wait for ae af lock"); }
    ...
    public void onCaptureCompleted(CameraCaptureSession session, CaptureRequest request, TotalCaptureResult result) {
        ...
        if (waitForFirstFrame) {
            if (waitForFirstFrameCallback != null) { waitForFirstFrameCallback.onFirstFrame(); }
...
*/
        if (result.get(CaptureResult.LENS_FOCUS_RANGE) != null) { focusRanges = result.get(CaptureResult.LENS_FOCUS_RANGE); }
        ...
        if (result.get(CaptureResult.CONTROL_AE_STATE)
            != null /*&& aeState != result.get(CaptureResult.CONTROL_AE_STATE)*/) {
        ...
        if (camera2Fragment.getParameterHandler().get(SettingKeys.ExposureLock) != null
            && result.get(CaptureResult.CONTROL_AE_LOCK) != null) {
        ...
        if (waitForAe_af_lock != null) {
            Log.d(TAG,
                  "ae locked: "
                      + aeAfLocker.getAeLock()
                      + " af locked: "
                      + aeAfLocker.getAfLock()
                      + " "
                      + Thread.currentThread().getId());
            waitForAe_af_lock.on_Ae_Af_Lock(aeAfLocker);
        }
        if (SettingsManager.get(SettingKeys.HISTOGRAM_STATS_QCOM).get()
            && result.get(CaptureResultQcom.HISTOGRAM_STATS) != null) {
    ...
    private void processDefaultFocus(TotalCaptureResult result) {
        if (result.get(CaptureResult.CONTROL_AF_STATE)
            != null /*&& afState != result.get(CaptureResult.CONTROL_AF_STATE)*/) {
            ...
            if (result.get(TotalCaptureResult.LENS_FOCUS_DISTANCE) != null
                && result.get(TotalCaptureResult.CONTROL_AF_MODE) != TotalCaptureResult.CONTROL_AF_MODE_OFF) {
                try {
                    focus_distance = result.get(TotalCaptureResult.LENS_FOCUS_DISTANCE);
                    camera2Fragment.getParameterHandler()
                        .get(SettingKeys.M_Focus)
                        .fireStringValueChanged(StringUtils.getMeterString(1 / focus_distance));
    ...
    private void processDefaultAEValues(TotalCaptureResult result, ParameterInterface expotime, ParameterInterface iso) {
        if (expotime != null && expotime.getViewState() == AbstractParameter.ViewState.Visible
            || expotime.getViewState() == AbstractParameter.ViewState.Disabled) {
            if (result != null && result.getKeys().size() > 0) {
                try {
                    ...
                    } else { expotime.fireStringValueChanged("1/60"); }
