...
public class AeManagerCamera2 extends AeManager {
    ...
    public void setExposureTime(int valueToSet, boolean setToCamera) {
        if (valueToSet > 0) {
            long val =
                AbstractManualShutter.getMilliSecondStringFromShutterString(manualExposureTime.getStringValues()[valueToSet])
                    * 1000;
            ...
            if (val > MAX_PREVIEW_EXPOSURETIME && !SettingsManager.getInstance()
                .GetCurrentModule()
            ...
            cameraUiWrapper.captureSessionHandler.SetPreviewParameterRepeating(CaptureRequest.SENSOR_EXPOSURE_TIME,
                                                                               val,
                                                                               setToCamera);
    ...
    public void setIso(int valueToSet, boolean setToCamera) {
        if (valueToSet == 0) {
            cameraUiWrapper.getParameterHandler()
                .get(SettingKeys.ExposureMode)
                .SetValue(FreedApplication.getContext().getString(R.string.on), setToCamera);
            ...
            if (activeAeState != AeStates.manual) {
                ...
                cameraUiWrapper.getParameterHandler()
                    .get(SettingKeys.ExposureMode)
                    .SetValue(FreedApplication.getContext().getString(R.string.off), setToCamera);
            }
            cameraUiWrapper.captureSessionHandler.SetParameterRepeating(CaptureRequest.SENSOR_SENSITIVITY,
                                                                        Integer.parseInt(manualIso.getStringValues()[valueToSet]),
                                                                        setToCamera);
    ...
    public void setExposureCompensation(int valueToSet, boolean setToCamera) {
        ...
        cameraUiWrapper.captureSessionHandler.SetParameterRepeating(CaptureRequest.CONTROL_AE_EXPOSURE_COMPENSATION,
                                                                    t,
                                                                    setToCamera);
    ...
    private void setToAuto() {
        ...
        if (SettingsManager.get(SettingKeys.FlashMode).isSupported()) {
            cameraWrapperInterface.getParameterHandler()
                .get(SettingKeys.FlashMode)
                .SetValue(cameraWrapperInterface.getParameterHandler().get(SettingKeys.FlashMode).GetStringValue(), true);
    ...
    private void setToManual() {
        ...
        if (cameraUiWrapper.getParameterHandler().get(SettingKeys.FlashMode) != null) {
            cameraUiWrapper.captureSessionHandler.SetParameterRepeating(CaptureRequest.FLASH_MODE,
