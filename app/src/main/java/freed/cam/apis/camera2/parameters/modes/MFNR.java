...
public class MFNR extends BaseModeApi2 {
    public MFNR(Camera2 cameraUiWrapper) {
        ...
        if (SettingsManager.get(SettingKeys.MFNR).isSupported()) { setViewState(ViewState.Visible); }
    }
    ...
    public void setValue(String valueToSet, boolean setToCamera) {
        ...
        if (SettingsManager.get(SettingKeys.MFNR).get()) {
            cameraUiWrapper.captureSessionHandler.SetParameterRepeating(CaptureRequest.NOISE_REDUCTION_MODE,
            ...
            cameraUiWrapper.getParameterHandler()
                .get(SettingKeys.Denoise)
                .SetValue(cameraUiWrapper.getParameterHandler().get(SettingKeys.Denoise).GetStringValue(), true);
