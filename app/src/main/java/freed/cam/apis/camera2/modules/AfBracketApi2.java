...
public class AfBracketApi2 extends PictureModuleApi2 {
    ...
    protected void onStartTakePicture() {
        ...
        cameraUiWrapper.captureSessionHandler.SetCaptureParameter(CaptureRequest.CONTROL_AF_MODE,
                                                                  CaptureRequest.CONTROL_AF_MODE_OFF);
        cameraUiWrapper.captureSessionHandler.SetPreviewParameter(CaptureRequest.CONTROL_AF_MODE,
                                                                  CaptureRequest.CONTROL_AF_MODE_OFF,
                                                                  true);
        int max = 0;
        try {
    ...
    protected void prepareCaptureBuilder(int captureNum) {
        cameraUiWrapper.captureSessionHandler.SetCaptureParameter(CaptureRequest.LENS_FOCUS_DISTANCE,
                                                                  manualFocus.getFloatValue(currentFocusPos));
        cameraUiWrapper.captureSessionHandler.SetPreviewParameter(CaptureRequest.LENS_FOCUS_DISTANCE,
                                                                  manualFocus.getFloatValue(currentFocusPos),
                                                                  true);
        Log.d(TAG,
              "prepareCaptureBuilder() focusCaptureRange:"
                  + focusCaptureRange
                  + " focusStep:"
                  + focusStep
                  + " currentFocusPos:"
                  ...
                  + manualFocus.getFloatValue(currentFocusPos)
                  + " :"
                  + manualFocus.getStringValue(currentFocusPos));
