...
public class CaptureSessionHandler {
    ...
    public CaptureSessionHandler(Camera2 cameraUiWrapper,
                                 CameraValuesChangedCaptureCallback cameraBackroundValuesChangedListner) {
        ...
        Display display =
            ((WindowManager) FreedApplication.getContext().getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay();
    ...
    public void CreatePreviewRequestBuilder() {
        if (cameraHolderApi2 == null || cameraHolderApi2.mCameraDevice == null) { return; }
        try {
    ...
    public void createImageCaptureRequestBuilder() {
        if (cameraHolderApi2 == null || cameraHolderApi2.mCameraDevice == null) { return; }
        try {
            mImageCaptureRequestBuilder =
                cameraHolderApi2.mCameraDevice.createCaptureRequest(CameraDevice.TEMPLATE_STILL_CAPTURE);
            if (SettingsManager.getInstance().getFrameWork() == Frameworks.HuaweiCamera2Ex) {
    ...
    public void AddSurface(Surface surface, boolean addtoPreviewRequestBuilder) {
        ...
        if (addtoPreviewRequestBuilder) {
            if (mPreviewRequestBuilder == null) { CreatePreviewRequestBuilder(); }
    ...
    public void RemoveSurface(Surface surface) {
        ...
        if (surfaces.contains(surface)) { surfaces.remove(surface); }
        try {
            if (mPreviewRequestBuilder != null) { mPreviewRequestBuilder.removeTarget(surface); }
    ...
    public void CreateCaptureSession() {
        ...
        if (cameraHolderApi2.mCameraDevice == null) { return; }
    ...
    public void CreateCustomCaptureSession(int OPMODE) {
        ...
        try {
            ...
            createCustomCaptureSession(cameraHolderApi2.mCameraDevice,
                                       null,
                                       outputConfigurations,
                                       OPMODE,
                                       previewStateCallBackRestart,
                                       handler);
        } catch (Exception ex) {
            Log.WriteEx(ex);
        }
    ...
    private void createCustomCaptureSession(CameraDevice cdv,
                                            InputConfiguration var1,
                                            List<OutputConfiguration> var2,
                                            int var3,
                                            CameraCaptureSession.StateCallback var4,
                                            Handler var5) {
        try {
            Method ccreateCustomCaptureSession =
                CameraDevice.class.getMethod("createCustomCaptureSession",
    ...
    public void StartRepeatingCaptureSession() {
        ...
        if (mCaptureSession == null || surfaces.size() == 0) { return; }
        try {
    ...
    public void StartHighspeedCaptureSession() {
        ...
        if (mCaptureSession == null || !isHighSpeedSession) { return; }
        try {
    ...
    public <T> void SetParameterRepeating(CaptureRequest.Key<T> key, T value, boolean setToCamera) {
        if (key != null && value != null) { Log.d(TAG, " SetParameterRepeating(" + key.getName() + " " + value + ")"); }
        if (mPreviewRequestBuilder == null) { return; }
        mPreviewRequestBuilder.set(key, value);
        if (mImageCaptureRequestBuilder != null) { mImageCaptureRequestBuilder.set(key, value); }
        if (setToCamera) {
            if (isHighSpeedSession) { StartHighspeedCaptureSession(); } else { StartRepeatingCaptureSession(); }
        }
    }
    ...
    public <T> void SetParameterRepeating(CaptureRequest.Key<T> key,
                                          T value,
    ...
    public <T> void SetParameter(CaptureRequest.Key<T> key, T value) {
        ...
        mPreviewRequestBuilder.set(key, value);
        if (isHighSpeedSession) { StartHighspeedCaptureSession(); } else {
            if (mImageCaptureRequestBuilder != null) { mImageCaptureRequestBuilder.set(key, value); }
    ...
    public <T> void SetCaptureParameter(CaptureRequest.Key<T> key, T value) {
        if (mImageCaptureRequestBuilder == null || mCaptureSession == null) { return; }
    ...
    public void StartAePrecapture() {
        SetPreviewParameter(CaptureRequest.CONTROL_AE_PRECAPTURE_TRIGGER,
                            CaptureRequest.CONTROL_AE_PRECAPTURE_TRIGGER_START,
                            false);
    }

    public <T> void SetFocusArea(CaptureRequest.Key<T> key, T value) {
        if (value != null) { Log.d(TAG, "Set :" + key.getName() + " to " + value.toString()); }
        if (isHighSpeedSession && Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            mPreviewRequestBuilder.set(key, value);
