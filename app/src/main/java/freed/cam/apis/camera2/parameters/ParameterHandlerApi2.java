...
public class ParameterHandlerApi2 extends AbstractParameterHandler<Camera2> {
    ...
    public void Init() {
        ...
        if (SettingsManager.get(SettingKeys.SceneMode).isSupported()) {
            add(SettingKeys.SceneMode,
                new BaseModeApi2(cameraUiWrapper, SettingKeys.SceneMode, CaptureRequest.CONTROL_SCENE_MODE));
        }
        if (SettingsManager.get(SettingKeys.AntiBandingMode).isSupported()) {
            add(SettingKeys.AntiBandingMode,
                new BaseModeApi2(cameraUiWrapper, SettingKeys.AntiBandingMode, CaptureRequest.CONTROL_AE_ANTIBANDING_MODE));
        }
        if (SettingsManager.get(SettingKeys.ColorMode).isSupported()) {
            add(SettingKeys.ColorMode,
                new BaseModeApi2(cameraUiWrapper, SettingKeys.ColorMode, CaptureRequest.CONTROL_EFFECT_MODE));
        }
        if (SettingsManager.get(SettingKeys.CONTROL_MODE).isSupported()) {
            add(SettingKeys.CONTROL_MODE,
                new BaseModeApi2(cameraUiWrapper, SettingKeys.CONTROL_MODE, CaptureRequest.CONTROL_MODE));
        }
        if (SettingsManager.get(SettingKeys.Denoise).isSupported()) {
            add(SettingKeys.Denoise,
                new BaseModeApi2(cameraUiWrapper, SettingKeys.Denoise, CaptureRequest.NOISE_REDUCTION_MODE));
        }
        ...
        if (SettingsManager.get(SettingKeys.OIS_MODE).isSupported()) {
            add(SettingKeys.OIS_MODE,
                new BaseModeApi2(cameraUiWrapper, SettingKeys.OIS_MODE, CaptureRequest.LENS_OPTICAL_STABILIZATION_MODE));
        }
        if (SettingsManager.get(SettingKeys.FocusMode).isSupported()) {
            add(SettingKeys.FocusMode,
                new FocusMode(cameraUiWrapper, SettingKeys.FocusMode, CaptureRequest.CONTROL_AF_MODE));
        }
        if (SettingsManager.get(SettingKeys.HOT_PIXEL_MODE).isSupported()) {
            add(SettingKeys.HOT_PIXEL_MODE,
                new BaseModeApi2(cameraUiWrapper, SettingKeys.HOT_PIXEL_MODE, CaptureRequest.HOT_PIXEL_MODE));
        }
        if (SettingsManager.get(SettingKeys.Ae_TargetFPS).isSupported()) {
            add(SettingKeys.Ae_TargetFPS,
                new AeTargetRangeApi2(cameraUiWrapper,
                                      SettingKeys.Ae_TargetFPS,
                                      CaptureRequest.CONTROL_AE_TARGET_FPS_RANGE));
        }

        if (SettingsManager.get(SettingKeys.dualPrimaryCameraMode).isSupported() && !SettingsManager.getInstance()
            .getIsFrontCamera()) {
            add(SettingKeys.dualPrimaryCameraMode,
                new DualCameraModeHuaweiApi2(cameraUiWrapper,
                                             SettingKeys.dualPrimaryCameraMode,
                                             CaptureRequestHuawei.HUAWEI_DUAL_SENSOR_MODE));
        }

        if (SettingsManager.get(SettingKeys.VideoStabilization).isSupported()) {
            add(SettingKeys.VideoStabilization,
                new BaseModeApi2(cameraUiWrapper,
                                 SettingKeys.VideoStabilization,
                                 CaptureRequest.CONTROL_VIDEO_STABILIZATION_MODE));
        }

        add(SettingKeys.JpegQuality, new JpegQualityModeApi2(cameraUiWrapper));
        if (SettingsManager.get(SettingKeys.MFNR).isSupported()) { add(SettingKeys.MFNR, new MFNR(cameraUiWrapper)); }
        ...
        add(SettingKeys.TONE_MAP_MODE,
            new BaseModeApi2(cameraUiWrapper, SettingKeys.TONE_MAP_MODE, CaptureRequest.TONEMAP_MODE));
        ...
        if (SettingsManager.get(SettingKeys.LensShade).isSupported()) {
            add(SettingKeys.LensShade,
                new BaseModeApi2(cameraUiWrapper, SettingKeys.LensShade, CaptureRequest.SHADING_MODE));
        }
    ...
    public void unregisterListners() {
        ...
        if (manualToneMapCurveApi2 != null) { manualToneMapCurveApi2.stopListning(); }
    }
    ...
    public void SetPictureOrientation(int orientation) {
        ...
        if (cameraHolder == null || cameraHolder.isWorking) { return; }
        try {
    ...
    public void SetAppSettingsToParameters() {
        ...
        if (SettingsManager.get(SettingKeys.TONE_MAP_MODE) != null
            ...
            VideoToneCurveProfile profile = SettingsManager.getInstance()
                .getVideoToneCurveProfiles()
                .get(SettingsManager.get(SettingKeys.TONE_CURVE_PARAMETER).get());
            if (profile != null) {
                ((ManualToneMapCurveApi2.ToneCurveParameter) cameraUiWrapper.getParameterHandler()
                    .get(SettingKeys.TONE_CURVE_PARAMETER)).setCurveToCamera(pointFtoFloatArray(profile.rgb));
            }
        }
    }
