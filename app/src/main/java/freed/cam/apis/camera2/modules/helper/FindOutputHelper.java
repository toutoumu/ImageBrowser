...
public class FindOutputHelper {
    ...
    public Output getHuaweiOutput(CameraHolderApi2 cameraHolder) {
        ...
        if (SettingsManager.get(SettingKeys.dualPrimaryCameraMode).isSupported()) {
            ...
            Log.d(TAG,
                  "secondary sensor size supported: " + SettingsManager.get(SettingKeys.secondarySensorSize).isSupported());
            //handel the first cam or the case that the second sensor has same size as first
            if (camera.equals(FreedApplication.getStringFromRessources(R.string.hw_dualcamera_Primary)) || camera.equals(
                FreedApplication.getStringFromRessources(R.string.hw_dualcamera_Dual))) {
                if (findColorOutput(cameraHolder, picFormat, output)) { return getStockOutput(cameraHolder); }
            } else if (camera.equals(FreedApplication.getStringFromRessources(R.string.hw_dualcamera_Secondary))) {
                if (findMonoOutput(cameraHolder, picFormat, output)) { return getStockOutput(cameraHolder); }
            }
        } else { return getStockOutput(cameraHolder); }
    ...
    private boolean findMonoOutput(CameraHolderApi2 cameraHolder, String picFormat, Output output) {
        ...
        if (output.raw_format == ImageFormat.RAW_SENSOR && !SettingsManager.get(SettingKeys.secondarySensorSize)
            .isSupported()) { return true; } else {
            ...
            if (picFormat.equals(FreedApplication.getStringFromRessources(R.string.pictureformat_dng16))
                ...
                int[] subsize =
                    cameraHolder.characteristics.get(CameraCharacteristicsHuawei.HUAWEI_SENCONDARY_SENSOR_PIXEL_ARRAY_SIZE);
    ...
    private void findRawFormat(CameraHolderApi2 cameraHolder, Output output) {
        try {
            if (CameraCharacteristicsHuawei.HUAWEI_RAW_FORMAT != null
                && cameraHolder.characteristics.get(CameraCharacteristicsHuawei.HUAWEI_RAW_FORMAT) != null) {
                output.raw_format =
                    cameraHolder.characteristics.get(CameraCharacteristicsHuawei.HUAWEI_RAW_FORMAT).intValue();
            } else { output.raw_format = ImageFormat.RAW_SENSOR; }
    ...
    public Output getStockOutput(CameraHolderApi2 cameraHolder) {
        ...
        if (picFormat.equals(FreedApplication.getStringFromRessources(R.string.pictureformat_dng16))
            ...
            if (SettingsManager.get(SettingKeys.RawSize).isSupported()) {
                ...
                largestImageSize = Collections.max(Arrays.asList(cameraHolder.map.getOutputSizes(ImageFormat.RAW_SENSOR)),
                                                   new CameraHolderApi2.CompareSizesByArea());
            ...
            largestImageSize = Collections.max(Arrays.asList(cameraHolder.map.getOutputSizes(ImageFormat.RAW12)),
                                               new CameraHolderApi2.CompareSizesByArea());
            output.raw_width = largestImageSize.getWidth();
            output.raw_height = largestImageSize.getHeight();
