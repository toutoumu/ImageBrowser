...
public class ManualFocus extends AbstractParameter<Camera2> {
    ...
    public void setValue(int valueToSet, boolean setToCamera) {
        ...
        if (valueToSet == 0) {
            ...
            cameraUiWrapper.getParameterHandler()
                .get(SettingKeys.FocusMode)
                .SetValue(SettingsManager.get(SettingKeys.FocusMode).get(), setToCamera);
        ...
        } else // set to manual
        {
            ...
            if (!cameraUiWrapper.getParameterHandler()
                .get(SettingKeys.FocusMode)
                .GetStringValue()
                .equals(FreedApplication.getStringFromRessources(R.string.off))) {
                //apply turn off direct to the capturesession, else it get stored in settings.
                cameraUiWrapper.getParameterHandler()
                    .get(SettingKeys.FocusMode)
                    .fireStringValueChanged(FreedApplication.getStringFromRessources(R.string.off));
                cameraUiWrapper.captureSessionHandler.SetParameter(CaptureRequest.CONTROL_AF_TRIGGER,
                                                                   CaptureRequest.CONTROL_AF_TRIGGER_CANCEL);
                cameraUiWrapper.captureSessionHandler.SetParameterRepeating(CaptureRequest.CONTROL_AF_MODE,
                                                                            CaptureRequest.CONTROL_AF_MODE_OFF,
                                                                            setToCamera);
            }
            applyAutoZoom();
            if (currentInt > focusvalues.getSize()) { currentInt = focusvalues.getSize() - 1; }
            float valtoset = focusvalues.getValue(currentInt);
            Log.d(TAG, "Set MF TO: " + valtoset + " ValueTOSET: " + valueToSet);
            cameraUiWrapper.captureSessionHandler.SetParameterRepeating(CaptureRequest.LENS_FOCUS_DISTANCE,
                                                                        valtoset,
                                                                        setToCamera);
        }
    }
    ...
    private void applyAutoZoom() {
        if (SettingsManager.get(SettingKeys.ZOOM_ON_MANUALFOCUS).isSupported()
            && SettingsManager.get(SettingKeys.ZOOM_ON_MANUALFOCUS).get()) {
