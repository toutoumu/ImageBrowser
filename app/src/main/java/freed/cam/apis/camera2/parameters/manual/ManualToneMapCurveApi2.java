...
public class ManualToneMapCurveApi2 implements EventBusLifeCycle {
    ...
    public void onToneMapModeChanged(ValueChangedEvent<String> valueChangedEvent) {
        ...
        if (valueChangedEvent.type != String.class) { return; }
        if (valueChangedEvent.key == SettingKeys.TONE_MAP_MODE) {
            Log.d(TAG, "onToneMapModeChanged");
            if (valueChangedEvent.newValue != null) { onStringValueChanged(valueChangedEvent.newValue); }
        }
    }
    ...
    public class Contrast extends AbstractParameter {
        ...
        public void setValue(int valueToSet, boolean setToCamera) {
            ...
            if (valueToSet == -1) {
                Log.d(TAG,
                      "Current TonemapMode:" + cameraUiWrapper.getParameterHandler()
                          .get(SettingKeys.TONE_MAP_MODE)
                          .GetValue());
                if (cameraUiWrapper.getParameterHandler()
                    .get(SettingKeys.TONE_MAP_MODE)
                    .GetStringValue()
                    .equals("CONTRAST_CURVE")) {
                ...
                if (!cameraUiWrapper.getParameterHandler()
                    .get(SettingKeys.TONE_MAP_MODE)
                    .GetStringValue()
                    .equals("CONTRAST_CURVE") && !firststart) {
    ...
    private void setTonemap() {
        float[] tonemap = {
            blackpoint[0], blackpoint[1], shadows[0], shadows[1], midtones[0], midtones[1], highlights[0], highlights[1],
        ...
        ((Camera2Fragment) cameraWrapperInterface).captureSessionHandler.SetParameterRepeating(CaptureRequest.TONEMAP_CURVE,
                                                                                               tonemapCurve,
                                                                                               true);
    }
