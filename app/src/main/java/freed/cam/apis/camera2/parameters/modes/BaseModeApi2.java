...
public class BaseModeApi2 extends AbstractParameter<Camera2> {
    ...
    protected void setValue(String valueToSet, boolean setToCamera) {
        ...
        if (parameterValues == null || parameterValues.size() == 0) { return; }
    ...
    public String GetStringValue() {
        if (parameterValues == null && captureSessionHandler == null) { return null; }
        try {
            ...
            for (Map.Entry s : maps)
                if (s.getValue().equals(i)) { return s.getKey().toString(); }
        } catch (NullPointerException ex) {
            Log.WriteEx(ex);
        }
