...
public class PictureModuleApi2 extends AbstractModuleApi2 implements RdyToSaveImg {
    ...
    public void DoWork() {
        ...
        if (!isWorking) { mBackgroundHandler.post(() -> takePicture()); } else if (isWorking) {
    ...
    public void startPreview() {
        ...
        Size previewSize = cameraUiWrapper.getSizeForPreviewDependingOnImageSize(ImageFormat.YUV_420_888,
                                                                                 output.jpeg_width,
                                                                                 output.jpeg_height);
        ...
        if (parameterHandler.get(SettingKeys.M_Burst) != null) {
            parameterHandler.get(SettingKeys.M_Burst)
                .fireStringValueChanged(parameterHandler.get(SettingKeys.M_Burst).GetStringValue());
        }
    ...
    private void preparePreviewTextureView(int orientationToSet, Size previewSize) {
       ...
       /* if (cameraUiWrapper.getPreviewSurface() != null)
        ...
        if (SettingsManager.getGlobal(SettingKeys.PREVIEW_POST_PROCESSING_MODE)
            .get()
            .equals(PreviewPostProcessingModes.RenderScript.name())) {
    ...
    protected void createImageCaptureListners() {
        ...
        if (captureType == CaptureType.Jpeg
            ...
            || captureType == CaptureType.JpegDng10) {
            byteImageCapture = new JpegCapture(new Size(output.jpeg_width, output.jpeg_height),
                                               false,
            ...
            byteImageCapture =
                new JpegCapture(smallestImageSize, false, cameraUiWrapper.getActivityInterface(), this, ".jpg", max_images);
        }
        ...
        if (captureType == CaptureType.Yuv) {
            ...
            ByteImageCapture yuvimgcapture = new ByteImageCapture(s,
        ...
        if (output.raw_format != 0 && isDngCapture()) {
            RawImageCapture rawImageCapture = new RawImageCapture(new Size(output.raw_width, output.raw_height),
                                                                  output.raw_format,
                                                                  false,
                                                                  cameraUiWrapper.getActivityInterface(),
                                                                  this,
            ...
            ByteImageCapture byteImageCapture1 = new ByteImageCapture(new Size(output.raw_width, output.raw_height),
                                                                      output.raw_format,
                                                                      false,
                                                                      cameraUiWrapper.getActivityInterface(),
                                                                      this,
    ...
    private boolean isDngCapture() {
        return captureType == CaptureType.JpegDng10
            || captureType == CaptureType.JpegDng16
    ...
    private void getCaptureType() {
        ...
        if (picFormat.equals(FreedApplication.getStringFromRessources(R.string.pictureformat_dng16)) || picFormat.equals(
            FreedApplication.getStringFromRessources(R.string.pictureformat_jpg_p_dng))) {
    ...
    protected void takePicture() {
        ...
        if (BurstCounter.getBurstCount() > 1) { isBurstCapture = true; } else { isBurstCapture = false; }
        onStartTakePicture();

        // FIXME: 2018/3/23 Camera2 拍摄的照片旋转90度问题
        cameraUiWrapper.getParameterHandler().SetPictureOrientation(cameraUiWrapper.getActivityInterface().getOrientation());

        if (SettingsManager.getInstance().hasCamera2Features() && (cameraUiWrapper.captureSessionHandler.getPreviewParameter(
            CaptureRequest.CONTROL_AE_MODE) != CaptureRequest.CONTROL_AE_MODE_OFF)
            || cameraUiWrapper.captureSessionHandler.getPreviewParameter(CaptureRequest.CONTROL_AF_MODE)
    ...
    private boolean isContAutoFocus() {
        return cameraUiWrapper.captureSessionHandler.getPreviewParameter(CaptureRequest.CONTROL_AF_MODE)
    ...
    private void startPreCapture() {
        ...
        // FIXME: 2021/6/15 Camera2 拍摄照片速度过慢的问题修复
        if (App.isLogin()) {
        ...
        cameraUiWrapper.cameraBackroundValuesChangedListner.setWaitForAe_af_lock(new CameraValuesChangedCaptureCallback.WaitForAe_Af_Lock() {
            ...
            public void on_Ae_Af_Lock(CameraValuesChangedCaptureCallback.AeAfLocker aeAfLocker) {
                // FIXME: 2021/6/15 Camera2 退到后台出问题了解决一下
                if (cameraUiWrapper.captureSessionHandler.getPreviewParameter(CaptureRequest.CONTROL_AF_MODE) == null) {
                    Timber.e("已经停止预览,这里就不执行了吧");
                    mBackgroundHandler.post(() -> {
                        Log.d(TAG, "cancel capture");
                        cameraUiWrapper.captureSessionHandler.cancelCapture();
                        finishCapture();
        ...
        if (cameraUiWrapper.captureSessionHandler.getPreviewParameter(CaptureRequest.CONTROL_AF_MODE)
            != CaptureRequest.CONTROL_AF_MODE_OFF) { cameraUiWrapper.captureSessionHandler.StartAePrecapture(); }
        if (isContAutoFocus()) {
            cameraUiWrapper.captureSessionHandler.SetPreviewParameter(CaptureRequest.CONTROL_AF_TRIGGER,
    ...
    protected void captureStillPicture() {
        ...
        if (cameraUiWrapper.getParameterHandler()
            .get(SettingKeys.LOCATION_MODE)
            .GetStringValue()
        ...
        for (int i = 0; i < captureController.getImageCaptures().size(); i++) {
            ...
            if (currentCaptureHolder instanceof StillImageCapture) {
                ...
                stillImageCapture.setToneMapProfile(((ToneMapChooser) cameraUiWrapper.getParameterHandler()
                    .get(SettingKeys.TONEMAP_SET)).getToneMap());
                ...
                if (currentLocation != null) { stillImageCapture.setLocation(currentLocation); }
        ...
        cameraUiWrapper.captureSessionHandler.SetCaptureParameter(CaptureRequest.JPEG_ORIENTATION,
    ...
    protected String getFileString() {
        if (BurstCounter.getBurstCount() > 1) {
            return cameraUiWrapper.getActivityInterface()
                .getFileListController()
                .getNewFilePath(SettingsManager.getInstance().GetWriteExternal(), "_" + BurstCounter.getImageCaptured());
        } else {
            return cameraUiWrapper.getActivityInterface()
                .getFileListController()
                .getNewFilePath(SettingsManager.getInstance().GetWriteExternal(), "");
        }
    }
    ...
    protected void finishCapture() {
        ...
        try {
            ...
            if (BurstCounter.getBurstCount() > BurstCounter.getImageCaptured()) {
                ...
                cameraUiWrapper.captureSessionHandler.getPreviewParameter(CaptureRequest.CONTROL_AE_MODE)
                    == CaptureRequest.CONTROL_AE_MODE_OFF &&
                cameraUiWrapper.captureSessionHandler.getPreviewParameter(CaptureRequest.SENSOR_EXPOSURE_TIME)
                    > AeManagerCamera2.MAX_PREVIEW_EXPOSURETIME) {
                cameraUiWrapper.captureSessionHandler.SetPreviewParameter(CaptureRequest.SENSOR_EXPOSURE_TIME,
                                                                          AeManagerCamera2.MAX_PREVIEW_EXPOSURETIME,
                                                                          true);
                cameraUiWrapper.captureSessionHandler.SetPreviewParameter(CaptureRequest.SENSOR_FRAME_DURATION,
                                                                          AeManagerCamera2.MAX_PREVIEW_EXPOSURETIME,
                                                                          true);
    ...
    private void onSesssionRdy() {
        Log.d(TAG,
              "onSessionRdy() ######################### Rdy to Start Preview, CAPTURE CYCLE DONE #####################");
    ...
    public void internalFireOnWorkDone(BaseHolder file) {
        Log.d(TAG,
              "internalFireOnWorkDone BurstCount:"
                  + BurstCounter.getBurstCount()
                  + " imageCount:"
                  + BurstCounter.getImageCaptured());
        ...
        if (isBurstCapture && BurstCounter.getBurstCount() == BurstCounter.getImageCaptured()) {
            ...
            filesSaved.clear();
        } else if (!isBurstCapture) { fireOnWorkFinish(file); }
    }
