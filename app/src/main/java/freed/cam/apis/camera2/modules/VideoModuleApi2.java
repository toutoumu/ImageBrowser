...
public class VideoModuleApi2 extends AbstractModuleApi2 {
    ...
    public void DoWork() {
        if (cameraUiWrapper.getActivityInterface()
            ...
            .isPermissionGranted(PermissionManager.Permissions.RecordAudio)) { startStopRecording(); } else {
    ...
    private void stopRecording() {
        ...
        if (opcodeProcessor != null && Build.VERSION.SDK_INT >= VERSION_CODES.N) { opcodeProcessor.stopRecording(); }
    ...
    public void startPreview() {
        ...
        if (currentVideoProfile.Mode != VideoMediaProfile.VideoMode.Highspeed) {
            if (currentVideoProfile.videoFrameWidth > 3840) {
                ...
                previewSize = getSizeForPreviewDependingOnImageSize(cameraHolder.map.getOutputSizes(ImageFormat.YUV_420_888),
                                                                    cameraHolder.characteristics,
                                                                    currentVideoProfile.videoFrameWidth,
                                                                    currentVideoProfile.videoFrameHeight);
            }
        } else {
            if (currentVideoProfile.videoFrameWidth > 3840) {
        ...
        if (active_op != OpCodes.off && Build.VERSION.SDK_INT >= VERSION_CODES.N) {
    ...
    public Size getSizeForPreviewDependingOnImageSize(Size[] choices,
        ...
        for (Size s : choices) {
            if (s.getWidth() <= cameraUiWrapper.captureSessionHandler.displaySize.x
                && s.getHeight() <= cameraUiWrapper.captureSessionHandler.displaySize.y
                && (double) s.getWidth() / s.getHeight() == ratio) { sizes.add(s); }
        }
        if (sizes.size() > 0) {
    ...
    private void startPreviewVideo() {
        String file = cameraUiWrapper.getActivityInterface()
        ...
        // FIXME: 2018/3/23 如果已经登录那么视频保存到指定目录
        if (App.isLogin()) {
            recordingFile = new FileHolder(new File(ConcealUtil.VIDEO_DIR.getPath()
                                                        + File.separator
                                                        + System.currentTimeMillis()
                                                        + ".mp8"), SettingsManager.getInstance().GetWriteExternal());
        }
        ...
        if (SettingsManager.getGlobal(SettingKeys.LOCATION_MODE)
            ...
            if (location != null) { videoRecorder.setLocation(location); }
        } else { videoRecorder.setLocation(null); }
        ...
        if (videoRecorder.prepare()) {
            ...
            if (active_op != OpCodes.off && Build.VERSION.SDK_INT >= VERSION_CODES.N) {
                ...
                } else { cameraUiWrapper.captureSessionHandler.CreateHighSpeedCaptureSession(recordingSessionCallback); }
            }
        } else {
            isRecording = false;
            changeCaptureState(ModuleHandlerAbstract.CaptureStates.video_recording_stop);
    ...
    private final StateCallback previewSessionCallback = new StateCallback() {
        ...
        public void onConfigured(@NonNull CameraCaptureSession session) {
            ...
            if (opcodeProcessor != null && Build.VERSION.SDK_INT >= VERSION_CODES.N) {
            ...
            cameraUiWrapper.captureSessionHandler.SetPreviewParameter(CaptureRequest.CONTROL_AE_TARGET_FPS_RANGE,
                                                                      fps,
                                                                      false);
    ...
    private int getHFRResIndex() {
        int index = -1;
        StreamConfigurationMap smap =
            cameraHolder.characteristics.get(CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP);
        Size sizes[] = smap.getHighSpeedVideoSizes();
        for (int i = 0; i < sizes.length; i++) {
            if (sizes[i].getWidth() == currentVideoProfile.videoFrameWidth
                && sizes[i].getHeight() == currentVideoProfile.videoFrameHeight) { index = i; }
        }
        return index;
    }
    ...
    private void applyQcomSettingsToSession(OpCodes active_op) {
        ...
        setQcomVideoHdr();
        if (opcodeProcessor != null && Build.VERSION.SDK_INT >= VERSION_CODES.N) { opcodeProcessor.applyOpCodeToSession(); }
    }
    ...
    private void setQcomVideoHdr() {
        if (SettingsManager.get(SettingKeys.QCOM_VIDEO_HDR10).isSupported()) {
            switch (currentVideoProfile.videoHdr) {
                case 0:
                    cameraUiWrapper.captureSessionHandler.SetPreviewParameter(CaptureRequestQcom.HDR10_VIDEO,
                                                                              CaptureRequestQcom.HDR10_VIDEO_OFF,
                                                                              false);
                    break;
                case 1:
                    cameraUiWrapper.captureSessionHandler.SetPreviewParameter(CaptureRequestQcom.HDR10_VIDEO,
                                                                              CaptureRequestQcom.HDR10_VIDEO_HLG,
                                                                              false);
                    break;
                case 2:
                    cameraUiWrapper.captureSessionHandler.SetPreviewParameter(CaptureRequestQcom.HDR10_VIDEO,
                                                                              CaptureRequestQcom.HDR10_VIDEO_PQ,
                                                                              false);
                    break;
            }
        }
