...
public class ManualIsoSony extends AbstractParameter {
    ...
    public void setValue(int valueToSet, boolean setToCamera) {
        ...
        if (currentInt == 0) {
            ...
            if (cameraUiWrapper.getParameterHandler().get(SettingKeys.M_ExposureTime).GetValue() == 0 && !parameters.get(
                "sony-ae-mode").equals("iso-prio")) {
                parameters.set("sony-ae-mode", "iso-prio");
            } else if (cameraUiWrapper.getParameterHandler().get(SettingKeys.M_ExposureTime).GetValue() > 0
                && !parameters.get(
                "sony-ae-mode").equals("manual")) { parameters.set("sony-ae-mode", "manual"); }
