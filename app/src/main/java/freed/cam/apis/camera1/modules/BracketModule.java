...
public class BracketModule extends PictureModule {
        ...
        mBackgroundHandler.post(() -> {
            ...
            if (picformat.equals(FreedApplication.getStringFromRessources(R.string.dng_)) || picformat.equals(
                ...
                if (zsl != null && zsl.getViewState() == AbstractParameter.ViewState.Visible
                    ...
                    && (SettingsManager.getInstance().getFrameWork() != Frameworks.MTK)) { zsl.SetValue("off", true); }
            }
    ...
    private void setExposureToCamera() {
        ...
        if (hdrCount == 0) {
            value = -2;
        } else if (hdrCount == 1) { value = 0; } else if (hdrCount == 2) { value = 2; }

        Log.d(TAG, "Set HDR Exposure to :" + value + "for image count " + hdrCount);
        int toset = value
            + cameraUiWrapper.getParameterHandler().get(SettingKeys.M_ExposureCompensation).getStringValues().length / 2;
    ...
    protected File getFile(String fileending) {
        return new File(cameraUiWrapper.getActivityInterface()
                            ...
                            .getNewFilePathBurst(SettingsManager.getInstance().GetWriteExternal(), fileending, hdrCount));
    }
