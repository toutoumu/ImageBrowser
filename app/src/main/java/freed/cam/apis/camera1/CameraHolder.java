...
public class CameraHolder extends CameraHolderAbstract implements CameraHolderInterfaceApi1 {
    ...
    public void StartFocus(final FocusEvents autoFocusCallback) {
        ...
        try {
            mCamera.autoFocus((success, camera) -> {
                if (mCamera == null) { return; }
                if (success) { mCamera.cancelAutoFocus(); }
    ...
    public void SetMeteringAreas(Rect meteringRect) {
        try {
            ...
            if (meteringRect != null) {
                meteringList.add(new Area(new Rect(meteringRect.left,
                                                   meteringRect.top,
                                                   meteringRect.right,
                                                   meteringRect.bottom), 100));
            }
            Parameters p = mCamera.getParameters();
            if (p.getMaxNumMeteringAreas() > 0) { p.setMeteringAreas(meteringList); }
    ...
    public void SetLocation(Location loc) {
        if (mCamera != null && loc != null) {
            ...
            if (loc.hasAltitude()) { paras.setGpsAltitude(loc.getAltitude()); }
