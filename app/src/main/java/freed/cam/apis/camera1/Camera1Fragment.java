 ...
 */
public class Camera1Fragment extends CameraFragmentAbstract<Camera1>
    implements ModuleChangedEvent, Preview.PreviewEvent, EventBusLifeCycle {
    ...
    public void onCameraClose(CameraStateEvents.CameraCloseEvent cameraCloseEvent) {
        if (camera != null) {
            if (camera.getFocusHandler() != null) { ((FocusHandler) camera.getFocusHandler()).stopListning(); }
            if (camera.getParameterHandler() != null) { camera.getParameterHandler().unregisterListners(); }
        }
        getPreview().close();
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        } else if (SettingsManager.getGlobal(SettingKeys.PREVIEW_POST_PROCESSING_MODE)
            .get()
            .equals(PreviewPostProcessingModes.OpenGL.name())) {
            getPreview().initPreview(PreviewPostProcessingModes.OpenGL, getContext(), histogramController);
        } else { getPreview().initPreview(PreviewPostProcessingModes.off, getContext(), histogramController); }
    ...
    public void onResume() {
        super.onResume();
        startListning();
        if (PreviewSurfaceRdy && !cameraIsOpen) { CameraThreadHandler.startCameraAsync(); }
    }
    ...
    public void startListning() {
        ...
        if (camera.getFocusHandler() != null) { ((FocusHandler) camera.getFocusHandler()).startListning(); }
    }
    ...
    public void onPreviewAvailable(SurfaceTexture surface, int width, int height) {
        ...
        if (!cameraIsOpen) { CameraThreadHandler.startCameraAsync(); } else { CameraThreadHandler.initCameraAsync(); }
    }
