...
public class ZoomManualParameter extends BaseManualParameter {
    public ZoomManualParameter(Parameters parameters, CameraWrapperInterface cameraUiWrapper, SettingKeys.Key key) {
        ...
        if (parameters.get(FreedApplication.getStringFromRessources(R.string.zoom_supported)) != null) {
            if (parameters.get(FreedApplication.getStringFromRessources(R.string.zoom_supported))
                .equals(FreedApplication.getStringFromRessources(R.string.true_))) {
                setViewState(ViewState.Visible);
                stringvalues = createStringArray(0,
                                                 Integer.parseInt(parameters.get(FreedApplication.getStringFromRessources(R.string.zoom_max))),
                                                 1);
                try {
