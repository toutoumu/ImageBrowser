...
public class PictureFormatHandler extends BaseModeParameter {
     ...
     */
    public PictureFormatHandler(Parameters parameters,
        ...
        if (SettingsManager.getInstance().getDngProfilesMap() != null
            && SettingsManager.getInstance().getDngProfilesMap().size() > 0) { dngprofilessupported = true; }
        ...
        if (rawSupported) {
            ...
            if (TextUtils.isEmpty(rawFormat) && rawFormats != null && rawFormats.length > 0) { rawFormat = rawFormats[0]; }
            ...
            } else { SettingsManager.get(SettingKeys.RAW_PICTURE_FORMAT_SETTING).setIsSupported(false); }
            if (!contains(SettingsManager.get(SettingKeys.RAW_PICTURE_FORMAT_SETTING).getValues(),
    ...
    private boolean contains(String[] arr, String tofind) {
        for (int i = 0; i < arr.length; i++) {
            if (arr[i].equals(tofind)) { return true; }
        }
        return false;
    }
    ...
    public class BayerFormat extends BaseModeParameter {
        ...
        public ViewState getViewState() {
            if (rawFormats != null && rawFormats.length > 0) { return ViewState.Visible; } else { return ViewState.Hidden; }
        }

        @Override
        public void SetValue(String valueToSet, boolean setToCam) {
            rawFormat = valueToSet;
            if (captureMode.equals(FreedApplication.getStringFromRessources(R.string.bayer_)) || captureMode.equals(
                FreedApplication.getStringFromRessources(R.string.dng_))) {
