...
public class ParametersHandler extends AbstractParameterHandler {
    ...
    private void initParameters() {
        ...
        if (SettingsManager.get(SettingKeys.WhiteBalanceMode).isSupported()) {
            add(SettingKeys.WhiteBalanceMode,
                new BaseModeParameter(cameraParameters, cameraUiWrapper, SettingKeys.WhiteBalanceMode));
        }

        if (SettingsManager.get(SettingKeys.ExposureMode).isSupported()) {
            add(SettingKeys.ExposureMode,
                new BaseModeParameter(cameraParameters, cameraUiWrapper, SettingKeys.ExposureMode));
        }
        ...
        if (SettingsManager.get(SettingKeys.AntiBandingMode).isSupported()) {
            add(SettingKeys.AntiBandingMode,
                new BaseModeParameter(cameraParameters, cameraUiWrapper, SettingKeys.AntiBandingMode));
        }

        if (SettingsManager.get(SettingKeys.ImagePostProcessing).isSupported()) {
            add(SettingKeys.ImagePostProcessing,
                new BaseModeParameter(cameraParameters, cameraUiWrapper, SettingKeys.ImagePostProcessing));
        }
        ...
        if (SettingsManager.get(SettingKeys.PreviewSize).isSupported()) {
            add(SettingKeys.PreviewSize,
                new PreviewSizeParameter(cameraParameters, cameraUiWrapper, SettingKeys.PreviewSize));
        }
        ...
        if (SettingsManager.get(SettingKeys.PreviewFpsRange).isSupported()) {
            add(SettingKeys.PreviewFpsRange,
                new BaseModeParameter(cameraParameters, cameraUiWrapper, SettingKeys.PreviewFpsRange));
        }

        if (SettingsManager.get(SettingKeys.PreviewFormat).isSupported()) {
            add(SettingKeys.PreviewFormat,
                new BaseModeParameter(cameraParameters, cameraUiWrapper, SettingKeys.PreviewFormat));
        }
        ...
        if (SettingsManager.get(SettingKeys.MemoryColorEnhancement).isSupported()) {
            add(SettingKeys.MemoryColorEnhancement,
                new BaseModeParameter(cameraParameters, cameraUiWrapper, SettingKeys.MemoryColorEnhancement));
        }
        ...
        if (SettingsManager.get(SettingKeys.VideoHighFramerate).isSupported()) {
            add(SettingKeys.VideoHighFramerate,
                new BaseModeParameter(cameraParameters, cameraUiWrapper, SettingKeys.VideoHighFramerate));
        }
        ...
        if (SettingsManager.get(SettingKeys.NonZslManualMode).isSupported()) {
            add(SettingKeys.NonZslManualMode,
                new BaseModeParameter(cameraParameters, cameraUiWrapper, SettingKeys.NonZslManualMode));
        }

        if (SettingsManager.get(SettingKeys.DigitalImageStabilization).isSupported()) {
            add(SettingKeys.DigitalImageStabilization,
                new BaseModeParameter(cameraParameters, cameraUiWrapper, SettingKeys.DigitalImageStabilization));
        }
        ...
        if (SettingsManager.get(SettingKeys.M_Focus).isSupported()) {
            if (SettingsManager.getInstance().getFrameWork() == Frameworks.MTK) {
                ...
                //htc mf
                if (SettingsManager.get(SettingKeys.M_Focus)
                    .getCamera1ParameterKEY()
                    .equals(FreedApplication.getStringFromRessources(R.string.focus))) {
                    add(SettingKeys.M_Focus,
                        new FocusManualParameterHTC(cameraParameters, cameraUiWrapper, SettingKeys.M_Focus));
                }
                //huawai mf
                else if (SettingsManager.get(SettingKeys.M_Focus)
                    .getCamera1ParameterKEY()
                    .equals(FreedApplication.getStringFromRessources(R.string.hw_manual_focus_step_value))) {
        ...
        if (SettingsManager.get(SettingKeys.M_Saturation).isSupported()) {
            add(SettingKeys.M_Saturation,
                new BaseManualParameter(cameraParameters, cameraUiWrapper, SettingKeys.M_Saturation));
        }

        if (SettingsManager.get(SettingKeys.M_Sharpness).isSupported()) {
            add(SettingKeys.M_Sharpness,
                new BaseManualParameter(cameraParameters, cameraUiWrapper, SettingKeys.M_Sharpness));
        }

        if (SettingsManager.get(SettingKeys.M_Brightness).isSupported()) {
            add(SettingKeys.M_Brightness,
                new BaseManualParameter(cameraParameters, cameraUiWrapper, SettingKeys.M_Brightness));
        }
        ...
        if (SettingsManager.getInstance().getFrameWork()
            == Frameworks.LG)//its needed else cam ignores manuals like shutter and iso
        { cameraParameters.set("lge-camera", "1"); } else if (SettingsManager.getInstance().getFrameWork()
            == Frameworks.MTK) {
        ...
        if (SettingsManager.get(SettingKeys.M_ExposureTime).isSupported()) {
            ...
            switch (type) {
                case SHUTTER_HTC:
                    ...
                    add(SettingKeys.M_ExposureTime,
                        new ShutterManualParameterHTC(cameraParameters, cameraUiWrapper, SettingKeys.M_ExposureTime));
                    break;
                case SHUTTER_QCOM_MILLISEC:
                    add(SettingKeys.M_ExposureTime,
                        new ExposureTime_MS(cameraUiWrapper, cameraParameters, SettingKeys.M_ExposureTime));
                    break;
                ...
                case SHUTTER_G2PRO:
                    add(SettingKeys.M_ExposureTime,
                        new ShutterManualG2pro(cameraParameters, cameraUiWrapper, SettingKeys.M_ExposureTime));
                    break;
                case SHUTTER_ZTE:
        ...
        if (SettingsManager.get(SettingKeys.M_ManualIso).isSupported()
            ...
            switch (SettingsManager.get(SettingKeys.M_ManualIso).getType()) {
                case ISOMANUAL_QCOM:
                    add(SettingKeys.M_ManualIso,
                        new BaseISOManual(cameraParameters, cameraUiWrapper, SettingKeys.M_ManualIso));
                    break;
                case ISOMANUAL_SONY:
                    add(SettingKeys.M_ManualIso,
                        new ManualIsoSony(cameraUiWrapper, cameraParameters, SettingKeys.M_ManualIso));
                    break;
                case ISOMANUAL_KRILLIN:
                    add(SettingKeys.M_ManualIso,
                        new ManualIsoKirin(cameraParameters, cameraUiWrapper, SettingKeys.M_ManualIso));
                    break;
        ...
        if (SettingsManager.get(SettingKeys.M_Whitebalance).isSupported()) {
            add(SettingKeys.M_Whitebalance,
                new BaseCCTManual(cameraParameters, cameraUiWrapper, SettingKeys.M_Whitebalance));
        }

        add(SettingKeys.M_ExposureCompensation,
            new ExposureManualParameter(cameraParameters, cameraUiWrapper, SettingKeys.M_ExposureCompensation));
        ...
        if (SettingsManager.get(SettingKeys.dualPrimaryCameraMode).isSupported()) {
            add(SettingKeys.dualPrimaryCameraMode,
                new BaseModeParameter(cameraParameters, cameraUiWrapper, SettingKeys.dualPrimaryCameraMode));
        }

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            add(SettingKeys.openCamera1Legacy,
                new LegacyMode(cameraUiWrapper, SettingsManager.get(SettingKeys.openCamera1Legacy)));
        }
    ...
    public void SetFocusAREA(Rect focusAreas) {
        if (SettingsManager.get(SettingKeys.useQcomFocus).get()) { setQcomFocus(focusAreas); } else {
    ...
    private void setAndroidFocus(Rect focusAreas) {
        ...
        } else { cameraParameters.setFocusAreas(null); }
        SetParametersToCamera(cameraParameters);
    }
    ...
    public void SetPictureOrientation(int orientation) {
        ...
        if (!SettingsManager.get(SettingKeys.orientationHack).get().equals("0") || SettingsManager.getInstance()
            .getIsFrontCamera()) {
    ...
    public float getCurrentExposuretime() {
        Parameters parameters = ((CameraHolder) cameraUiWrapper.getCameraHolder()).GetCameraParameters();
        if (SettingsManager.getInstance().getFrameWork() == Frameworks.MTK) {
            if (parameters.get(FreedApplication.getStringFromRessources(R.string.eng_capture_shutter_speed)) != null) {
                if (Float.parseFloat(parameters.get(FreedApplication.getStringFromRessources(R.string.eng_capture_shutter_speed)))
                    == 0) {
                    return 0;
                } else {
                    return Float.parseFloat(parameters.get(FreedApplication.getStringFromRessources(R.string.eng_capture_shutter_speed)))
                        / 1000000;
                }
                ...
                if (Float.parseFloat(parameters.get(FreedApplication.getStringFromRessources(R.string.cap_ss))) == 0) {
                    ...
                    return Float.parseFloat(parameters.get(FreedApplication.getStringFromRessources(R.string.cap_ss)))
                        / 1000000;
                }
            } else { return 0; }
        } else {
            if (parameters.get(FreedApplication.getStringFromRessources(R.string.cur_exposure_time)) != null) {
                return Float.parseFloat(parameters.get(FreedApplication.getStringFromRessources(R.string.cur_exposure_time)))
                    / 1000;
            }
        }
        return 0;
    ...
    public int getCurrentIso() {
        ...
        if (SettingsManager.getInstance().getFrameWork() == Frameworks.MTK) {
            if (parameters.get(FreedApplication.getStringFromRessources(R.string.eng_capture_sensor_gain)) != null) {
                if (Integer.parseInt(parameters.get(FreedApplication.getStringFromRessources(R.string.eng_capture_sensor_gain)))
                    == 0) {
                    return 0;
                }
                return Integer.parseInt(parameters.get(FreedApplication.getStringFromRessources(R.string.eng_capture_sensor_gain)))
                    / 256 * 100;
                ...
                return Integer.parseInt(parameters.get(FreedApplication.getStringFromRessources(R.string.cap_isp_g))) / 256
                    * 100;
            } else { return 0; }
        } else {
