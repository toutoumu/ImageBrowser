...
public class AeManagerLgCamera1 extends AeManager {
    ...
    public void setExposureTime(int valueToSet, boolean setToCamera) {
        parameters.set(FreedApplication.getStringFromRessources(R.string.lg_shutterspeed),
                       manualExposureTime.getStringValues()[valueToSet]);
    ...
    public void setIso(int valueToSet, boolean setToCamera) {
        if (valueToSet == 0) {
            ...
            parameters.set(FreedApplication.getStringFromRessources(R.string.lg_iso),
                           manualIso.getStringValues()[valueToSet]);
            ((ParametersHandler) cameraWrapperInterface.getParameterHandler()).SetParametersToCamera(parameters);
    ...
    private void setToAuto() {
        ...
        if (!t.equals(FreedApplication.getStringFromRessources(R.string.iso100_))) {
            cameraWrapperInterface.getParameterHandler()
                .get(SettingKeys.IsoMode)
                .SetValue(FreedApplication.getStringFromRessources(R.string.iso100_), true);
        } else {
            cameraWrapperInterface.getParameterHandler()
                .get(SettingKeys.IsoMode)
                .SetValue(FreedApplication.getStringFromRessources(R.string.auto_), true);
        }
    ...
    private void startReadingMeta() {
        ...
        FreeDPool.Execute(() -> {
            while (readMetaData) {
                try {
                    cameraWrapperInterface.getParameterHandler()
                        .get(SettingKeys.M_ExposureTime)
                        .fireStringValueChanged("1/" + (int) cameraWrapperInterface.getParameterHandler()
                            .getCurrentExposuretime());
                    cameraWrapperInterface.getParameterHandler()
                        .get(SettingKeys.M_ManualIso)
                        .fireStringValueChanged(cameraWrapperInterface.getParameterHandler().getCurrentIso() + "");
