...
public class VideoModule extends AbstractVideoModule {
    ...
    protected void initRecorder() {
        ...
        if (currentProfile == null) {
            VideoProfilesParameter videoProfilesG3Parameter =
                (VideoProfilesParameter) cameraUiWrapper.getParameterHandler().get(SettingKeys.VideoProfiles);
    ...
    private void loadProfileSpecificParameters() {
        ...
        Log.d(TAG,
              "LoadProfile: "
                  ...
                  + " Size: "
                  + currentProfile.videoFrameWidth
                  + "/"
                  + currentProfile.videoFrameHeight);
        if (currentProfile.Mode == VideoMode.Highspeed) {
                    ...
                    for (String fpz : SettingsManager.get(SettingKeys.PreviewFPS).getValues()) {
                        if (Integer.parseInt(fpz) == currentProfile.videoFrameRate) {
                            cameraUiWrapper.getParameterHandler()
                                .get(SettingKeys.PreviewFPS)
                                .SetValue(currentProfile.videoFrameRate + "", false);
                        }
                    }
                }
            }
            ...
            try {
                if (SettingsManager.get(SettingKeys.PreviewFpsRange).isSupported()) {
                    ...
                    if (currentProfile.videoFrameRate <= 30) {
                        cameraUiWrapper.getParameterHandler()
                            .get(SettingKeys.PreviewFpsRange)
                            .SetValue(String.valueOf(currentProfile.videoFrameRate * 1000) + "," + String.valueOf(
                                currentProfile.videoFrameRate * 1000), true);
                    }
                }
            ...
            if (currentProfile.videoFrameHeight >= 2160) {
                ...
                if (!SettingsManager.getInstance().hasCamera2Features()
                    && StringUtils.arrayContainsString(SettingsManager.get(SettingKeys.PreviewFormat).getValues(),
                                                       "nv12-venus")) {
            ...
            if (SettingsManager.get(SettingKeys.VideoHighFramerate).isSupported()) {
                cameraUiWrapper.getParameterHandler()
                    .get(SettingKeys.VideoHighFramerate)
    ...
    private void loadDefaultHighspeed() {
        ...
        if (!SettingsManager.getInstance().hasCamera2Features() && StringUtils.arrayContainsString(SettingsManager.get(
            SettingKeys.PreviewFormat).getValues(), "nv12-venus")) {
    ...
    private void disable_mce_dis_vs_denoise() {
        ...
        if (cameraUiWrapper.getParameterHandler().get(SettingKeys.MemoryColorEnhancement) != null && SettingsManager.get(
            SettingKeys.MemoryColorEnhancement).isSupported()) {
            cameraUiWrapper.getParameterHandler().get(SettingKeys.MemoryColorEnhancement).SetValue("disable", false);
        }
        Log.d(TAG, "disable_dis");
        if (cameraUiWrapper.getParameterHandler().get(SettingKeys.DigitalImageStabilization) != null && SettingsManager.get(
            SettingKeys.DigitalImageStabilization).isSupported()) {
            cameraUiWrapper.getParameterHandler().get(SettingKeys.DigitalImageStabilization).SetValue("disable", false);
        }
        Log.d(TAG, "disable_vs");
        if (cameraUiWrapper.getParameterHandler().get(SettingKeys.VideoStabilization) != null && SettingsManager.get(
            SettingKeys.VideoStabilization).isSupported()) {
        ...
        if (cameraUiWrapper.getParameterHandler().get(SettingKeys.Denoise) != null
            && cameraUiWrapper.getParameterHandler().get(SettingKeys.Denoise).getViewState()
            == AbstractParameter.ViewState.Visible) {
    ...
    private void loadMtkHighspeed() {
        ...
        if (Arrays.toString(cameraUiWrapper.getParameterHandler().get(SettingKeys.PreviewFPS).getStringValues())
            .contains(currentProfile.videoFrameRate + "")) {
            cameraUiWrapper.getParameterHandler()
                .get(SettingKeys.PreviewFPS)
                .SetValue(currentProfile.videoFrameRate + "", false);

            if (cameraUiWrapper.getParameterHandler().get(SettingKeys.VideoHighFramerate) != null
                && cameraUiWrapper.getParameterHandler().get(SettingKeys.VideoHighFramerate).getViewState()
                == AbstractParameter.ViewState.Visible) {
                cameraUiWrapper.getParameterHandler()
                    .get(SettingKeys.VideoHighFramerate)
                    .SetValue(currentProfile.videoFrameRate + "", false);
                cameraUiWrapper.getParameterHandler()
                    .get(SettingKeys.PreviewFPS)
                    .SetValue(currentProfile.videoFrameRate + "", true);
            }
        }
    }
