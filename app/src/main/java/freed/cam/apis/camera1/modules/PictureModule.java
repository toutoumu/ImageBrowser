...
public class PictureModule extends ModuleAbstract<Camera1> implements Camera.PictureCallback {
    ...
    public void DoWork() {
        ...
        mBackgroundHandler.post(() -> {
            ...
            if (picformat.equals(FreedApplication.getStringFromRessources(R.string.dng_)) || picformat.equals(
                ...
                if (SettingsManager.get(SettingKeys.ZSL).isSupported()
                    ...
                    Log.d(TAG,
                          "ZSL state after turning it off:" + cameraUiWrapper.getParameterHandler()
                              .get(SettingKeys.ZSL)
                              .GetValue());
                }
            }
            cameraUiWrapper.getParameterHandler()
                .SetPictureOrientation(cameraUiWrapper.getActivityInterface().getOrientation());
    ...
    public void InitModule() {
        ...
        if (cameraUiWrapper.getParameterHandler() == null) { return; }
        ...
        if (SettingsManager.get(SettingKeys.VideoHDR).isSupported() && !videohdr.GetStringValue()
            .equals(FreedApplication.getStringFromRessources(R.string.off_))) {
    ...
    private void createPreview() {
        ...
        if (!SettingsManager.getGlobal(SettingKeys.PREVIEW_POST_PROCESSING_MODE)
            .get()
            .equals(PreviewPostProcessingModes.off.name())) {
            ...
            cameraUiWrapper.getParameterHandler()
                .get(SettingKeys.PreviewSize)
                .SetValue(size.width + "x" + size.height, false);
            ...
            } else { ((CameraHolder) cameraHolder).setTextureView(cameraUiWrapper.getPreview().getSurfaceTexture()); }
    ...
    protected void startPreview() {
        ...
        if (cameraHolder.GetCameraParameters().getAutoExposureLock()) {
            cameraUiWrapper.getParameterHandler()
                .get(SettingKeys.ExposureLock)
                .SetValue(FreedApplication.getStringFromRessources(R.string.false_), true);
            cameraUiWrapper.getParameterHandler()
                .get(SettingKeys.ExposureLock)
                .SetValue(FreedApplication.getStringFromRessources(R.string.true_), true);
        }
        if (SettingsManager.get(SettingKeys.needRestartAfterCapture).get()) {
            MotoPreviewResetLogic();
        } else { cameraHolder.StartPreview(); }
    }
    ...
    protected void saveImage(byte[] data, String picFormat) {
        ...
        if (picFormat.equals(FileEnding.DNG)) { saveDng(data, toSave); } else {
            saveJpeg(data, toSave);
        }
        if (SettingsManager.getInstance().isZteAe()) { ShutterResetLogic(); }
    ...
    private String getFileEnding(String picFormat) {
        if (picFormat.equals(FreedApplication.getStringFromRessources(R.string.jpeg_))) { return ".jpg"; } else if (picFormat
            .equals("jps")) { return ".jps"; } else if ((picFormat.equals(FileEnding.BAYER)
            || picFormat.equals(FileEnding.RAW))) {
            return ".bayer";
        } else if (picFormat.contains(FileEnding.DNG)) { return ".dng"; }
        return "";
    }

    protected File getFile(String fileending) {
        if (isBurstCapture) {
            return new File(cameraUiWrapper.getActivityInterface()
                                .getFileListController()
                                .getNewFilePathBurst(SettingsManager.getInstance().GetWriteExternal(),
                                                     fileending,
                                                     burstcount));
        } else {
            return new File(cameraUiWrapper.getActivityInterface()
                                .getFileListController()
                                .getNewFilePath(SettingsManager.getInstance().GetWriteExternal(), fileending));
        }
    }
    ...
    protected void saveDng(byte[] data, File file) {
        ...
        if (wbct != null && wbct.getViewState() == AbstractParameter.ViewState.Visible) {
            ...
            if (wb.equals(FreedApplication.getStringFromRessources(R.string.auto_))) { wb = null; }
        ...
        } else { task.setOrientation(cameraUiWrapper.getActivityInterface().getOrientation()); }
        task.setFilePath(file, SettingsManager.getInstance().GetWriteExternal());
