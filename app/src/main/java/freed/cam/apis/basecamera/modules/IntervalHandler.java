...
public class IntervalHandler {
    ...
    public void StartInterval() {
        ...
        if (sleep.contains(" sec")) { sleepTimeBetweenCaptures = Integer.parseInt(sleep.replace(" sec", "")) * 1000; }
        if (sleep.contains(" min")) { sleepTimeBetweenCaptures = Integer.parseInt(sleep.replace(" min", "")) * 60 * 1000; }

        String duration = SettingsManager.get(SettingKeys.INTERVAL_DURATION).get();
        if (duration.equals("∞")) { fullIntervalCaptureDuration = 0; } else if (duration.contains(" min")) {
    ...
    private void startInterval() {
        ...
        intervalBackgroundThread = new Thread(() ->
                                              {
                                                  Log.d(TAG,
                                                        "Started IntervalThread" + " " + Thread.currentThread().getName());
                                                  working = true;
                                                  ...
                                                  while (!Thread.currentThread().isInterrupted()
                                                      ...
                                                      Log.d(TAG,
                                                            "capturetimeover:"
                                                                ...
                                                                + " captureTime:"
                                                                + captureTime
                                                                + " timeleftToNextCapture:"
                                                                + timeGoneTillNextCapture
                                                                + " sleepTimeBetweenCapture:"
                                                                + sleepTimeBetweenCaptures);
                                                      if (timeGoneTillNextCapture < sleepTimeBetweenCaptures / 1000) {
                                                          ...
                                                          if (captureTime > 0) { sleep = 1000 - captureTime; } else {
                                                              sleep = 0;
                                                          }
                                                      ...
                                                      if (captureTimeOver) { picmodule.IntervalCaptureIsDone(); }
                                                  }
                                                  working = false;
                                                  Log.d(TAG,
                                                        "Stopped IntervalThread" + " " + Thread.currentThread().getName());
                                              });
