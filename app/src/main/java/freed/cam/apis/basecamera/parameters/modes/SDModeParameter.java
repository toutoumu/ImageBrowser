...
public class SDModeParameter extends AbstractParameter {
    ...
    public ViewState getViewState() {
        try {
            if (VERSION.SDK_INT <= VERSION_CODES.LOLLIPOP) {
                ...
                if (file.exists()) { return ViewState.Visible; }
            } else { return ViewState.Visible; }
        } catch (Exception ex) {
    ...
    public String GetStringValue() {
        if (SettingsManager.getInstance().GetWriteExternal()) { return external; } else { return internal; }
    }
