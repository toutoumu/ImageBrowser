...
public abstract class AbstractParameterHandler<C extends CameraWrapperInterface> implements ParameterHandler {
    ...
    protected AbstractParameterHandler(C cameraUiWrapper) {
        ...
        add(SettingsManager.PREVIEW_POST_PROCESSING_MODE,
            new EnableRenderScriptMode(SettingsManager.PREVIEW_POST_PROCESSING_MODE));
        if (!SettingsManager.getGlobal(SettingsManager.PREVIEW_POST_PROCESSING_MODE)
            .get()
            .equals(PreviewPostProcessingModes.off.name())) {

            add(SettingsManager.FOCUSPEAK_COLOR,
                new FocusPeakColorMode(cameraUiWrapper.getPreview(), SettingsManager.FOCUSPEAK_COLOR));
        ...
        if (SettingsManager.get(parametertolook) instanceof SettingMode) {
            ...
            Log.d(TAG,
                  "setAppSettingsToCamera "
                      + FreedApplication.getStringFromRessources(parametertolook.getRessourcesStringID())
                      + " isSupported:"
                      + settingMode.isSupported());
            if (settingMode != null
                && settingMode.isSupported()
                && parameter != null
                && parameter.GetStringValue() != null) {
                if (TextUtils.isEmpty(settingMode.get())) { return; }
                ...
                Log.d(TAG,
                      "set "
                          ...
                          + " to :"
                          + toset);
                if (TextUtils.isEmpty(toset) || toset.equals("none")) { settingMode.set(parameter.GetStringValue()); } else {
