...
public class VideoRecorder {
    ...
    public boolean prepare() {
        ...
        if (previewSurface != null) { mediaRecorder.setPreviewDisplay(previewSurface); }
        try {
        ...
        try {
            if (this.currentVideoProfile.duration != 0) { mediaRecorder.setMaxDuration(this.currentVideoProfile.duration); }
        ...
        if (location != null) { mediaRecorder.setLocation((float) location.getLatitude(), (float) location.getLongitude()); }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O
            && this.currentVideoProfile.level != -1
            && this.currentVideoProfile.profile != -1) {
        ...
        switch (this.currentVideoProfile.Mode) {
            ...
            case Timelapse:
                ...
                } else { SettingsManager.get(SettingKeys.TIMELAPSE_FRAMES).set(String.valueOf(frame)); }
    ...
    private void setRecorderFilePath() {
        BaseHolder baseHolder =
            cameraWrapperInterface.getActivityInterface().getFileListController().getNewMovieFileHolder(recordingFile);
        try {
