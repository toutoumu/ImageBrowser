...
public class CameraFragmentManager {
    ...
    public CameraFragmentManager(FragmentManager fragmentManager,
                                 int fragmentHolderId,
                                 Context context,
    ...
    public void switchCameraFragment() {
        Log.d(TAG, "BackgroundHandler is null: "
            + (backgroundHandlerThread.getThread() == null)
            +
            " features detected: "
            + SettingsManager.getInstance().getAreFeaturesDetected()
            + " app version changed: "
            + SettingsManager.getInstance().appVersionHasChanged());
        if ((!SettingsManager.getInstance().getAreFeaturesDetected() || SettingsManager.getInstance()
            .appVersionHasChanged())) {
            Log.d(TAG, "load featuredetector");
            if (cameraFragment != null) { unloadCameraFragment(); }
            loadFeatureDetector();
        } else {
            if (cameraFragment == null) {
