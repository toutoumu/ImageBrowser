...
public class SettingsLoader {
    ...
    public void loadSettings(SettingLayout settingLayout, File appdata) {
        ...
        if (configFile.exists()) {
            try {
                ...
                settingLayout.showHelpOverlayOnStart =
                    xmlElement.findChild(XmlUtil.SHOW_HELPOVERLAY_ONSTART).getBooleanValue();
                ...
                if (settingLayout.extSdFolderUri.equals("null")) { settingLayout.extSdFolderUri = null; }
                try {
    ...
    private void addSettingElement(HashMap<SettingKeys.Key, SettingInterface> hashMap, XmlElement profile) {
        ...
        if (foundKey == null) { throw new NullPointerException("found key is null"); }
    ...
    private SettingKeys.Key findKey(String val) {
        ...
        for (SettingKeys.Key k : key)
            if (FreedApplication.getStringFromRessources(k.getRessourcesStringID()).equals(val)) { return k; }
        return null;
    }
}
