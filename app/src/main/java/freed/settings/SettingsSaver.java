...
public class SettingsSaver {
    ...
    public void saveSettings(SettingLayout settingLayout, File appdata) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(configFile));) {
            ...
            XmlUtil.writeTagWithValue(writer,
                                      XmlUtil.HAS_CAMERA2_FEATURES,
                                      String.valueOf(settingLayout.hasCamera2Features));
            XmlUtil.writeTagWithValue(writer,
                                      XmlUtil.ARE_FEATURES_DETECTED,
                                      String.valueOf(settingLayout.areFeaturesDetected));
            XmlUtil.writeTagWithValue(writer, XmlUtil.WRITE_TO_EXTERNALSD, String.valueOf(settingLayout.writeToExternalSD));
            XmlUtil.writeTagWithValue(writer,
                                      XmlUtil.SHOW_HELPOVERLAY_ONSTART,
                                      String.valueOf(settingLayout.showHelpOverlayOnStart));
    ...
    private void writeApiNode(BufferedWriter writer, SettingLayout.CameraId camera) throws IOException {
        ...
        if (camera.camera_ids != null) { writeCameraIds(writer, camera.camera_ids); }
        if (camera.cameraid_settings != null) {
            writeCameraSettings(writer, camera.cameraid_settings);
        }
    }

    private void writeCameraSettings(BufferedWriter writer,
                                     HashMap<Integer, SettingLayout.CameraId.CameraSettings> cameraid_settings)
        throws IOException {
    ...
    private void writeCameraIdSettings(BufferedWriter writer,
                                       HashMap<SettingKeys.Key, SettingInterface> stringVideoMediaProfileHashMap)
        throws IOException {
