...
public class SettingsStorage {
    ...
    public void setApiVideoMediaProfiles(HashMap<String, VideoMediaProfile> value) {
        mediaProfilesManager.addMediaProfilesToApiAndCamera(settings.active_api,
                                                            settings.api_hashmap.get(settings.active_api).active_camera,
                                                            value);
    }

    public HashMap<String, VideoMediaProfile> getApiVideoMediaProfiles() {
        return mediaProfilesManager.getMediaProfilesForApiAndCamera(settings.active_api,
                                                                    settings.api_hashmap.get(settings.active_api).active_camera);
    }
    ...
    public void setActiveCamera(int id) {
        ...
        if (settings.api_hashmap.get(settings.active_api).cameraid_settings.get(id) == null) {
            settings.api_hashmap.get(settings.active_api).cameraid_settings.put(id,
                                                                                new SettingLayout.CameraId.CameraSettings());
        }
    }
    ...
    public void setActiveCameraIds(int[] ids) {
        ...
        for (int i = 0; i < ids.length; i++)
            if (settings.api_hashmap.get(settings.active_api).cameraid_settings.get(i) == null) {
                settings.api_hashmap.get(settings.active_api).cameraid_settings.put(i,
                                                                                    new SettingLayout.CameraId.CameraSettings());
            }
    }
