...
public class SettingsManager extends SettingKeys implements SettingsManagerInterface {
    ...
    public void save() {
        if (settingsStorage != null) { settingsStorage.save(); }
    ...
    public void init() {
        ...
        if (ext != null && ext.exists()) { settingsStorage = new SettingsStorage(ext); } else {
    ...
    private void parseXml() {
        ...
        if (settingsStorage.getDevice() == null || TextUtils.isEmpty(settingsStorage.getDevice())) {
            ...
            parser.parseAndFindSupportedDevice(FreedApplication.getContext().getResources(),
                                               matrixes,
                                               settingsStorage.appdataFolder);
    ...
    private void loadOpCodes() {
        new Thread(() -> {
            try {
                File op2 = new File(settingsStorage.appdataFolder.getAbsolutePath()
                                        + "/"
                                        + settingsStorage.getActiveCamera()
                                        + "opc2.bin");
                File op3 = new File(settingsStorage.appdataFolder.getAbsolutePath()
                                        + "/"
                                        + settingsStorage.getActiveCamera()
                                        + "opc3.bin");
                if (op2.exists() || op3.exists()) { opCode = new OpCode(op2, op3); } else { opCode = null; }
