...
public abstract class BaseHolder {
    ...
    public BaseHolder(String name, long lastmodified, boolean isFolder, boolean isSDCard) {
        ...
        if (name.toLowerCase().endsWith(StringUtils.FileEnding.BAYER)) { fileformat = FileListController.FormatTypes.raw; }
        if (name.toLowerCase().endsWith(StringUtils.FileEnding.DNG)) { fileformat = FileListController.FormatTypes.dng; }
        if (name.toLowerCase().endsWith(StringUtils.FileEnding.RAW)) { fileformat = FileListController.FormatTypes.raw; }
        if (name.toLowerCase().endsWith(StringUtils.FileEnding.JPG)) { fileformat = FileListController.FormatTypes.jpg; }
        if (name.toLowerCase().endsWith(StringUtils.FileEnding.JPS)) { fileformat = FileListController.FormatTypes.jps; }
        if (name.toLowerCase().endsWith(StringUtils.FileEnding.MP4)) { fileformat = FileListController.FormatTypes.mp4; }
    }
    ...
    public void setToMediaRecorder(MediaRecorder recorder, ActivityInterface activityInterface)
