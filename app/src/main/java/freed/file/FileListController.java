...
public class FileListController {
    ...
    public void setNotifyFilesChanged(NotifyFilesChanged notifyFilesChanged) {
        if (!notifyFilesChangedList.contains(notifyFilesChanged)) { notifyFilesChangedList.add(notifyFilesChanged); }
    }
   ...
   /* public StorageFileManager getStorageFileManager()
    ...
    public void loadDefaultFiles() {
        try {
            ...
            if (!needStorageAccessFrameWork) { LoadDCIMDirs(); } else { files = mediaStoreController.getFolders(); }

            DocumentFile documentFile = getFreeDcamDocumentFolder();
            if (documentFile != null) {
                DocumentHolder documentHolder = new DocumentHolder(documentFile.getName(),
    ...
    public void LoadFolder(BaseHolder fileHolder, FormatTypes types) {
        ...
        synchronized (filesLock) {
            if (fileHolder instanceof DocumentHolder) {
                new DocumentFileController().readFilesFromFolder((DocumentHolder) fileHolder,
                                                                 files,
                                                                 types,
                                                                 fileHolder.isExternalSD());
            } else {
                if (!needStorageAccessFrameWork) {
                    files.clear();
                    storageFileManager.readFilesFromFolder(((FileHolder) fileHolder).getFile(),
                                                           files,
                                                           types,
                                                           fileHolder.isExternalSD());
                } else {
                    ...
                    if (types != FormatTypes.all) {
                        for (BaseHolder fh : tmplist) {
                            if (fh.getName() != null) {
                                if (fh.getName().endsWith("jpg") && types == FormatTypes.jpg) { files.add(fh); }
                                if (fh.getName().endsWith("jps") && types == FormatTypes.jps) { files.add(fh); }
                                if (fh.getName().endsWith("dng") && types == FormatTypes.dng) { files.add(fh); }
                                if (fh.getName().endsWith("bayer") && types == FormatTypes.raw) { files.add(fh); }
                                if (fh.getName().endsWith("mp4") && types == FormatTypes.mp4) { files.add(fh); }
                            }
                        }
                    } else { files = tmplist; }
                    SortFileHolder(files);
    ...
    public static DocumentFile getExternalSdDocumentFile(Context context) {
        ...
        if (extSdFolder == null || TextUtils.isEmpty(extSdFolder)) { return null; }
    ...
    public boolean DeleteFile(BaseHolder file) {
        ...
        for (int i = 0; i < getFiles().size(); i++) {
            if (getFiles().get(i).getName().equals(file.getName())) { fireNotifyFilesDeleted(i); }
        }
        synchronized (filesLock) {
        ...
        synchronized (filesLock) {
            if (files.get(0) instanceof FileHolder) {
                for (BaseHolder f : files)
                    deleteFile(f);
            } else { mediaStoreController.deleteFiles(files); }
            fireNotifyFilesChanged();
        }
    }
    ...
    public BaseHolder getNewImgFileHolder(File file) {
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.LOLLIPOP || (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP
            && !SettingsManager.getInstance().GetWriteExternal()
            && !FileListController.needStorageAccessFrameWork)) {
            ...
            return new UriHolder(wr.getUri(),
                                 file.getName(),
                                 0,
                                 wr.lastModified(),
                                 wr.isDirectory(),
    ...
    private void checkFileExists(File fileName) {
        ...
        if (fileName.getParentFile() == null) { return; }
    ...
    public String getNewFilePath(boolean external, String fileending) {
        if (needStorageAccessFrameWork) { return mediaStoreController.getNewFilePath(external, fileending); } else {
    ...
    public String getNewFilePathBurst(boolean external, String fileending, int count) {
        ...
        } else { return storageFileManager.getNewFilePathBurst(external, fileending, count); }
    }
}
