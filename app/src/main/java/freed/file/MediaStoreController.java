...
public class MediaStoreController {
    ...
    private Uri getUri() {
        ...
        } else { return MediaStore.Images.Media.getContentUri(MediaStore.VOLUME_EXTERNAL_PRIMARY); }
    }

    private Uri getVideoUri() {
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.P) {
            return MediaStore.Video.Media.getContentUri(MediaStore.VOLUME_EXTERNAL);
        } else { return MediaStore.Video.Media.getContentUri(MediaStore.VOLUME_EXTERNAL_PRIMARY); }
    }
    ...
    public List<BaseHolder> getFolders() {
        ...
        for (String name : folders)
            if (name != null) { fileHolders.add(new UriHolder(null, name, 0, 0, true, false)); }
        return fileHolders;
    }
    ...
    private void getImages(List<BaseHolder> fileHolders) {
        ...
        try (Cursor cursor = context.getContentResolver().query(
            ...
            while (cursor.moveToNext()) {
                ...
                Uri contentUri = ContentUris.withAppendedId(
                    MediaStore.Images.Media.EXTERNAL_CONTENT_URI, id);
                if (name != null) { fileHolders.add(new UriHolder(contentUri, name, id, datetaken, false, false)); }
            }
        } catch (SecurityException ex) {
            Log.WriteEx(ex);
        }
    ...
    public void deleteFiles(List<BaseHolder> baseHolders) {
        ...
        for (BaseHolder baseHolder : baseHolders) {
            ...
            contentProviderOperation = ContentProviderOperation.newDelete(uriHolder.getMediaStoreUri())
                .withSelection(MediaStore.Images.Media._ID + " = ?", new String[] { String.valueOf(uriHolder.getID()) })
                .build();
