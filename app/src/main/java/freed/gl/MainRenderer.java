...
public class MainRenderer implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener {
    ...
    public SurfaceTexture getmSTexture() {
        if (cameraInputTextureHolder != null) { return cameraInputTextureHolder.getmSTexture(); }
        return null;
    }
    ...
    private int getGlesVersion() {
        final ActivityManager activityManager =
            (ActivityManager) FreedApplication.getContext().getSystemService(Context.ACTIVITY_SERVICE);
        final ConfigurationInfo configurationInfo = activityManager.getDeviceConfigurationInfo();
        if (configurationInfo.reqGlEsVersion >= 0x20000 && configurationInfo.reqGlEsVersion <= 0x30000) { return 2; }
        if (configurationInfo.reqGlEsVersion >= 0x30000) { return 3; } else { return 1; }
    }
}
