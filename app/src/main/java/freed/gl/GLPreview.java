...
public class GLPreview extends GLSurfaceView {
    ...
    public void fireOnSurfaceTextureAvailable(SurfaceTexture surfaceTexture, int w, int h) {
        if (surfaceTextureListener != null) { surfaceTextureListener.onSurfaceTextureAvailable(surfaceTexture, w, h); }
    }
    ...
    private void applyProgram() {
        ...
        } else if (zebra_enabled) { setPreviewProcessors(PreviewProcessors.Zebra); } else if (focuspeak_enabled) {
            setPreviewProcessors(PreviewProcessors.FocusPeak);
        } else { setPreviewProcessors(PreviewProcessors.Normal); }
    }
}
