...
public class Keys {
    ...
    public ArrayList C(Class serial, CameraMetadata cameraMetadata) {
        if (Build.VERSION.SDK_INT < 26) { return getAllVendorKeys((Class) serial); }
    ...
    public ArrayList D(Class serial, CameraMetadata object) {
        try {
            ...
            return (ArrayList) Class.forName("android.hardware.camera2.impl.CameraMetadataNative")
                .getMethod("getAllVendorKeys", Class.class)
                .invoke(object, serial);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
