...
public class ReflectionHelper {
    ...
    public void logClass(Class classtoDump, int depth) {
        ...
        builder.append(getTab(depth)
                           + getAccessType(classtoDump.getModifiers())
                           + "class "
                           + classtoDump.getSimpleName()
                           + " {\r\n");
    ...
    private String createMethodLogString(Method method, int depth) {
        ...
        for (Class c : parametertypes) {
            if (parametertypes[parametertypes.length - 1] == c) { ret.append(c.getSimpleName()); } else {
    ...
    private String createFieldLogString(Field field, int depth) {
        ...
        if (Modifier.isStatic(mod)) {
            try {
                ...
                if (!publicfield) { field.setAccessible(true); }
                ret += " = " + field.get(null);
                if (!publicfield) { field.setAccessible(false); }
            } catch (IllegalAccessException e) {
                e.printStackTrace();
            }
        }
    ...
    private String getAccessType(int mod) {
        String ret = "";
        if (Modifier.isPrivate(mod)) { ret = "private "; }
        if (Modifier.isProtected(mod)) { ret += "protected "; }
        if (Modifier.isPublic(mod)) { ret += "public "; }
        if (Modifier.isAbstract(mod)) { ret += "abstract "; }
        if (Modifier.isFinal(mod)) { ret = "final "; }
        if (Modifier.isStatic(mod)) { ret += "static "; }
        if (Modifier.isNative(mod)) { ret += "native "; }
        if (Modifier.isSynchronized(mod)) { ret += "synchronized "; }
        if (Modifier.isStrict(mod)) { ret += "strict "; }
        return ret;
    }
}
